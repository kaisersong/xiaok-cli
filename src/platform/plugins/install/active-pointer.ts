...
export function activePointerPath(paths: InstallPaths, name: string): string {
...
function toPluginsRelative(paths: InstallPaths, absolute: string): string {
...
function fromPluginsRelative(paths: InstallPaths, value: unknown, label: string): string {
...
export async function switchActivePluginPointer(
  paths: InstallPaths,
  input: ActivePluginPointerInput,
  now: () => Date = () => new Date(),
): Promise<{ previous: ActivePluginPointer | null }> {
  const pointerFile = activePointerPath(paths, input.name);
  mkdirSync(paths.activeDir, { recursive: true });

  let previous: ActivePluginPointer | null = null;
  ...
  const tempFile = join(
    paths.activeDir,
    `.${input.name}.${process.pid}.${randomBytes(6).toString('hex')}.tmp`,
  );
...
function flushDirectory(dir: string): void {
...
export function readActivePluginPointer(paths: InstallPaths, name: string): ActivePluginPointer {
  const pointerFile = activePointerPath(paths, name);
  let raw: Record<string, unknown>;
  try {
    raw = JSON.parse(readFileSync(pointerFile, 'utf8')) as Record<string, unknown>;
  } catch (error) {
    throw new Error(`Plugin pointer ${pointerFile} is unreadable: ${(error as Error).message}`);
  }
  ...
  const probe = raw.probe as ProbeResult | undefined;
...
export interface ResolvedManagedPlugins {
  entries: Array<{ name: string; pointer: ActivePluginPointer }>;
  invalid: Array<{ name: string; reason: string }>;
}

export function resolveManagedPlugins(pluginsDir: string): ResolvedManagedPlugins {
  ...
  const entries: ResolvedManagedPlugins['entries'] = [];
  const invalid: ResolvedManagedPlugins['invalid'] = [];

  if (!existsSync(paths.activeDir)) return { entries, invalid };
  ...
  for (const file of readdirSync(paths.activeDir).sort()) {
    ...
    try {
      ...
      invalid.push({ name, reason: (error as Error).message });
    }
  }
...
export function removeActivePluginPointer(paths: InstallPaths, name: string): boolean {
  const pointerFile = activePointerPath(paths, name);
  if (!existsSync(pointerFile)) return false;
  unlinkSync(pointerFile);
  flushDirectory(paths.activeDir);
  return true;
}

export function pruneManagedVersions(paths: InstallPaths, name: string, keepDigests: string[]): string[] {
  return pruneDigestDirectories(join(paths.managedDir, name), keepDigests);
}

export function prunePluginRuntimeVersions(paths: InstallPaths, name: string, keepDigests: string[]): string[] {
  return pruneDigestDirectories(join(paths.runtimesDir, name), keepDigests);
}

function pruneDigestDirectories(rootDir: string, keepDigests: string[]): string[] {
...
export async function acquirePluginLock(
  paths: InstallPaths,
  name: string,
  options: AcquirePluginLockOptions = {},
): Promise<PluginLock> {
  assertValidPluginName(name);
  ...
  const claim = (): PluginLock | null => {
    ...
    return {
      async release() {
        try {
          const held = JSON.parse(readFileSync(lockFile, 'utf8')) as { token?: string };
  ...
  let held: { pid?: unknown; createdAt?: unknown };
  try {
    held = JSON.parse(readFileSync(lockFile, 'utf8')) as { pid?: unknown; createdAt?: unknown };
  } catch {
    throw new Error(
      `Plugin "${name}" has an unreadable install lock at ${lockFile}. Remove it manually after confirming no install is running.`,
    );
  }
  ...
  if (age < staleAfterMs) {
    throw new Error(
      `Another install of plugin "${name}" is in progress (lock from pid ${pid} is ${Math.max(0, Math.round(age / 1000))}s old)`,
    );
  }
