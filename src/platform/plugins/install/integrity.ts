...
export function isSupportedGitMode(mode: string): boolean {
  return SUPPORTED_MODES.has(mode);
}

export function computeGitTreeSha256(entries: GitObjectEntry[]): string {
 ...
 */
export function detectPathConflicts(paths: string[]): string[] {
...
export function assertPluginRelativePath(root: string, value: string, label = 'path'): string {
