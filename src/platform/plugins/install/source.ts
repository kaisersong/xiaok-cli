...
export function resolveInstallPaths(pluginsDir: string): InstallPaths {
  return {
    pluginsDir,
...
export function resolveDefaultPluginsDir(env: NodeJS.ProcessEnv = process.env): string {
...
export interface CommandRunner {
  ...
  hashStdout(
    ...
    args: string[],
    options?: RunCommandOptions,
  ): Promise<{ code: number; sha256: string; bytes: number; stderr: string }>;
}

const DEFAULT_TIMEOUT_MS = 120_000;
const STDERR_LIMIT = 8192;

type CollectMode = 'string' | 'buffer' | 'hash';
...
export function createRecordingRunner(
  ...
  const record = (command: string, args: string[], options?: RunCommandOptions) => {
 ...
 */
export function createGitEnv(baseEnv: NodeJS.ProcessEnv = process.env, allowLocalSource = false): Record<string, string> {
  const env: Record<string, string> = {};
  for (const [key, value] of Object.entries(baseEnv)) {
    if (value === undefined) continue;
    if (key.startsWith('GIT_')) continue;
    if (key === 'SSH_ASKPASS' || key === 'SSH_AUTH_SOCK') continue;
    env[key] = value;
  }
...
function resolveCloneUrl(entry: TrustedRegistryPlugin, options: StagePluginSourceOptions): string {
...
function parseTreeRecords(stdout: string, pluginPathPrefix: string): TreeRecord[] {
...
function assertSafeSymlinkTarget(pluginRelPath: string, target: string): void {
...
function listActualPaths(root: string): string[] {
  const found: string[] = [];
...
export async function stagePluginSource(options: StagePluginSourceOptions): Promise<StagedPluginSource> {
  ...
  try {
    if (!reuse) {
      const init = await runner.run(
        'git',
        [...gitConfigArgs, 'init', '--quiet', '--initial-branch=xiaok-install', repoDir],
        { env, timeoutMs },
      );
      if (init.code !== 0) {
      ...
      if (fetch.code !== 0) {
        throw new Error(
          `git fetch of pinned commit ${commit} failed (exit ${fetch.code}): ${fetch.stderr.trim()}`,
        );
      }
    ...
    for (const record of records) {
      ...
      const hashed = await runner.hashStdout(
        'git',
        [...gitConfigArgs, '-C', repoDir, 'cat-file', 'blob', record.oid],
        { env, timeoutMs },
      );
      if (hashed.code !== 0) {
    ...
    if (digest !== entry.source.treeSha256) {
      throw new Error(
        `Plugin source digest mismatch for "${entry.name}": registry declares treeSha256 ${entry.source.treeSha256} but commit ${commit} yields ${digest}`,
      );
    }
    ...
    for (const item of entries) {
      ...
      if (actual !== item.contentSha256) {
        throw new Error(
          `Checked-out bytes of "${item.path}" do not match the verified Git object (expected ${item.contentSha256}, got ${actual})`,
        );
      }
    }
...
function materializeSymlink(
  pluginDir: string,
  absolute: string,
  pluginRelPath: string,
  target: string,
  expected: Map<string, GitObjectEntry>,
  platform: NodeJS.Platform,
): void {
  const linkDir = dirname(pluginRelPath);
  const targetRel = relative(
    pluginDir,
    resolve(pluginDir, linkDir === '.' ? '' : linkDir, target),
  )
    .split(sep)
    .join('/');
  ...
  if (!targetEntry || targetEntry.mode === GIT_MODE_SYMLINK) {
    throw new Error(
      `Symlink "${pluginRelPath}" cannot be materialized on Windows because "${targetRel}" is not a verified regular file`,
    );
  }
