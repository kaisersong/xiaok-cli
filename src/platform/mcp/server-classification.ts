...
export interface McpClassificationEntry {
  match: { pluginName?: string; name: string };
...
const BUILT_IN_RAW: McpClassificationEntry[] = [
  {
...
export const BUILT_IN_MCP_CLASSIFICATIONS: readonly McpClassificationEntry[] = deepFreeze(
  BUILT_IN_RAW,
);

const DEFAULT_POLICY: McpServerPolicy = Object.freeze({
  activation: { mode: 'eager' } as McpActivation,
  disposeOwnership: 'owned-child' as McpDisposeOwnership,
  diagnostics: Object.freeze([]) as readonly McpDiagnosticTag[],
  reason: '',
 ...
 */
export function validateRegistry(registry: readonly McpClassificationEntry[]): void {
  for (const entry of registry) {
    if (entry.policy.activation.mode === 'lazy' && entry.policy.activation.adapter === 'cua-computer-use-wrapper') {
      ...
      if (!okPluginName || !okServerName) {
        throw new Error(
          `cua-computer-use-wrapper adapter only allowed for ${OFFICIAL_CUA.pluginName}/${OFFICIAL_CUA.name}, got ${entry.match.pluginName ?? '<any>'}/${entry.match.name}`,
        );
      }
    }
  }
...
function entryMatches(entry: McpClassificationEntry, server: NamedMcpServerConfig): boolean {
  if (entry.match.name !== server.name) return false;
  if (entry.match.pluginName !== undefined) {
    if (server.source?.origin !== 'plugin') return false;
    if (server.source.pluginName !== entry.match.pluginName) return false;
  }
  return true;
}

function isOfficialCuaIdentity(server: NamedMcpServerConfig): boolean {
  return (
...
function classifyByLegacyManifest(server: NamedMcpServerConfig): McpServerPolicy {
...
export function classifyMcpServer(
  ...
  if (matches.length > 1) {
    throw new Error(
      `Ambiguous MCP classification for "${server.name}": ${matches.length} entries match`,
    );
  }
  if (matches.length === 1) {
