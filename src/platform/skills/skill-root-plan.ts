...
export const NONRESERVED_PRIORITY: Record<Exclude<SkillProvenance, 'reserved'>, number> = Object.freeze({
  project: 0,
  global: 1,
  plugin: 2,
  builtin: 3,
...
export type SkillPlanDiagnostic =
  | {
    ...
    path: string; key: string; provenance: SkillProvenance; consumer: SkillConsumer;
  }
  | { code: 'legacy_skill_name_conflict'; winner: string; loser: string; key: string }
  | { code: 'denied_reserved_skill_impersonation'; path: string; key: string; consumer: SkillConsumer };

export class TrustedSkillContractConflictError extends Error {
  readonly code = 'trusted_skill_contract_conflict';

  constructor(readonly key: string, readonly paths: readonly string[]) {
...
function keysOf(candidate: SkillCandidate): string[] {
  return [candidate.name, ...candidate.aliases];
}

function comparePriority(a: SkillCandidate, b: SkillCandidate): number {
  const pa = NONRESERVED_PRIORITY[a.provenance as Exclude<SkillProvenance, 'reserved'>] ?? 99;
  const pb = NONRESERVED_PRIORITY[b.provenance as Exclude<SkillProvenance, 'reserved'>] ?? 99;
 ...
 */
export function reduceSkillPlan(plan: SkillRootPlan, candidates: readonly SkillCandidate[]): SkillPlanResult {
  const diagnostics: SkillPlanDiagnostic[] = [];
  ...
  const reservedKeyOwner = new Map<string, SkillCandidate>();
  ...
  const accepted: SkillCandidate[] = [...reserved];
  const claimed = new Map<string, SkillCandidate>(reservedKeyOwner);
  ...
  for (const candidate of nonReserved) {
    ...
    for (const key of keys) claimed.set(key, candidate);
...
export function assertPlanIsClosed(plan: SkillRootPlan, expected: readonly SkillProvenance[]): void {
