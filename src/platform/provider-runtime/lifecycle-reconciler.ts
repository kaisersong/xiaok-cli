...
import {
  ...
  type ProviderResourceMode,
} from './provider-slot-directory.js';
import { componentInstanceKeyOf, type ComponentInstanceKey } from './types.js';
...
interface ComponentRecord {
  readonly spec: PluginComponentSpec;
  desiredActive: boolean;
  desiredRevision: number;
  runtimeState: ComponentRuntimeState;
  committedGeneration: string | null;
  preparingGeneration: string | null;
  lastError?: string;
  blockedBy?: string;
  cleanupStatus?: string;
  inFlight: Promise<void> | null;
  generationSeq: number;
  pendingHealth: HealthSignal[];
}
...
export class LifecycleReconciler {
  ...
  constructor(
    readonly slots: ProviderSlotDirectory = new ProviderSlotDirectory(),
  ...
  register(spec: PluginComponentSpec): void {
    if (this.components.has(spec.id)) throw new Error(`component already registered: ${spec.id}`);
  ...
  projection(componentId: string): ComponentProjection {
    ...
    const actions: Array<'enable' | 'disable' | 'retry'> = [];
    if (record.spec.activation === 'user') actions.push(record.desiredActive ? 'disable' : 'enable');
    if (record.desiredActive && !ready) actions.push('retry');
    return {
      componentId,
  ...
  async setDesiredActive(componentId: string, desiredActive: boolean, requestSource: 'user'): Promise<void> {
    if (requestSource !== 'user') throw new Error('desired state mutation requires requestSource "user"');
    const record = this.require(componentId);
  ...
  async reconcileFromPersistedState(input: Record<string, boolean>): Promise<void> {
  ...
  async retry(componentId: string, requestSource: 'user'): Promise<void> {
    if (requestSource !== 'user') throw new Error('retry requires requestSource "user"');
    const record = this.require(componentId);
    if (!record.desiredActive) return; // a disabled component is not activated by retry
    record.desiredRevision += 1;
    await this.reconcile(componentId);
  }
  ...
  async applyPendingHealthSignals(componentId: string): Promise<void> {
    const record = this.require(componentId);
    ...
    if (record.desiredActive) await this.reconcile(componentId);
  }

  /** Single-flight per component; extra calls coalesce onto the running attempt. */
  async reconcile(componentId: string): Promise<void> {
    const record = this.require(componentId);
    if (record.inFlight) {
      await record.inFlight;
      // Requested revision may have moved while we waited; run once more.
      if (this.needsWork(record)) await this.reconcile(componentId);
      return;
    }
  ...
  private needsWork(record: ComponentRecord): boolean {
    ...
    if (!record.desiredActive && hasCommitted) return true;
    return false;
  }

  private async runReconcile(record: ComponentRecord): Promise<void> {
    if (!record.desiredActive) {
      if (record.committedGeneration) await this.retireCommitted(record, 'user_disabled');
      else this.setState(record, 'inactive');
      return;
    }
    if (record.committedGeneration) return; // already active
   ...
   */
  async replaceGeneration(componentId: string): Promise<{ replaced: boolean }> {
    const record = this.require(componentId);
  ...
  private async activate(record: ComponentRecord): Promise<void> {
    const startRevision = record.desiredRevision;
    record.generationSeq += 1;
    const provider = componentInstanceKeyOf(record.spec.id, `gen-${record.generationSeq}`);
    record.preparingGeneration = provider.generationId;
  ...
  private async retireCommitted(record: ComponentRecord, reason: string): Promise<void> {
  ...
  private setState(record: ComponentRecord, state: ComponentRuntimeState): void {
    record.runtimeState = state;
    this.projectionRevision += 1;
    this.hooks.onProjection?.(this.projection(record.spec.id));
  }

  private require(componentId: string): ComponentRecord {
    const record = this.components.get(componentId);
    if (!record) throw new Error(`unknown component: ${componentId}`);
    return record;
  }
}
