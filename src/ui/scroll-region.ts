...
export class ScrollRegionManager {
  ...
  private lastActivityRow: number | null = null;
  private lastActivityRows = 0;
  private lastInputPrompt = '';
  private lastSummaryLine = '';
  private lastStatusLine = '';
  private lastInputValue = '';
  private lastInputCursor = 0;
  private lastOverlayRenderRows = 0;
  ...
  private lastInputRenderRows = 1 + INPUT_PADDING_ROWS + INPUT_BOTTOM_PADDING_ROWS;
  ...
  private lastOverlayClearStartRow = 0;
  ...
  private countFooterLineRows(text: string): number {
    if (!text) {
      return 0;
    }
  ...
  private getSummaryReserveRows(summaryLine = this.lastSummaryLine): number {
    return this.countFooterLineRows(summaryLine);
  }

  private getSummaryStartRow(inputStartRow: number, summaryLine = this.lastSummaryLine): number {
  ...
  private getScrollBottom(): number {
    return this.getScrollBottomForLayout(
      this.lastInputRenderRows,
      this.lastOverlayRenderRows,
      this.lastSummaryLine,
    );
  }

  private getActivityRow(): number {
  ...
  private isRendererPermissionOverlayActive(): boolean {
    return this._activeOverlayKind === 'permission' && this._activeOverlayOwner === 'renderer';
  }

  private clearActiveOverlayPrompt(): void {
  ...
  private getClearScreenRowSequence(row: number): string {
    return `\x1b[${row};1H${CLEAR_LINE}`;
  }

  private composeActivityLineRender(activityLine: string): string {
  ...
  private composeActivityClear(rowShift = 0): string {
    let output = '';
    if (this.lastActivityRow !== null) {
      for (let index = 0; index < this.lastActivityRows; index++) {
        ...
        for (const row of new Set([oldRow, oldRow - rowShift])) {
          if (row >= 1 && row <= this.config.rows) output += `${MOVE_TO_ROW.replace('%d', String(row))}${CLEAR_LINE}`;
        }
      }
    }
  ...
  private clearPromptChromeRows(): void {
    const fallbackStart = this.getInputStartRow();
    const fallbackEnd = this.getStatusBarRow();
    const cachedStart = Math.min(
      this.lastFooterClearStartRow || fallbackStart,
      this.lastOverlayClearStartRow || fallbackStart,
      fallbackStart,
    );
  ...
  private clearSummaryChromeRows(): void {
  ...
  updateSize(rows: number, columns: number): void {
    ...
    if (this.active) {
      ...
      if (rowShift > 0) {
        for (let row = Math.max(1, this.lastFooterClearStartRow - rowShift); row <= rows; row++) this.clearScreenRow(row);
      }
  ...
  private renderFooterFrame(
    ...
    if (this.lastActivityRows > 1) this.stream.write(this.composeActivityClear());
    ...
    const previousScrollBottom = this.getScrollBottomForLayout(
      previousInputRows,
      previousOverlayRows,
      previousSummaryLine,
    );
    ...
    if (options?.summaryLine !== undefined) this.lastSummaryLine = options.summaryLine;
    ...
    const previousOverlayStartRow = previousOverlayRows > 0
      ? Math.max(
        1,
        previousInputStartRow - previousOverlayRows - this.config.gapHeight - this.getSummaryReserveRows(previousSummaryLine),
      )
      : previousInputStartRow;
    if (previousOverlayRows > 0) {
      this.lastOverlayClearStartRow = Math.min(
        this.lastOverlayClearStartRow || previousOverlayStartRow,
    ...
    const previousFooterClearStartRow = Math.max(
      1,
      previousInputStartRow - this.config.gapHeight - this.getSummaryReserveRows(previousSummaryLine),
    );
  ...
  private renderOverlayPromptFrame(frame: ScrollPromptFrame): void {
    ...
    const previousOverlayStartRow = Math.max(
      1,
      this.getInputStartRow(previousInputRows) - this.config.gapHeight - this.getSummaryReserveRows() - previousOverlayRows,
    );
    const previousFooterStartRow = Math.max(
      1,
    ...
    this.lastFooterClearEndRow = statusBarRow;
    this.lastOverlayClearStartRow = Math.min(
      this.lastOverlayClearStartRow || clearStartRow,
      clearStartRow,
    );
    if (isPermissionOverlay) {
   ...
   */
  clearLastInput(options?: { renderFooter?: boolean; inputPrompt?: string }): void {
    this.lastInputValue = '';
    this.lastInputCursor = 0;
   ...
   */
  private setScrollRegion(scrollEnd = this.getScrollBottom()): void {
  ...
  writeAtContentCursor(text: string, options?: { clearPromptChrome?: boolean }): void {
    if (!this.active) {
