import type { MultiAgentEvent, MultiAgentSnapshot } from '../ai/agents/multi-agent-coordinator.js';
import { publicAgentSummary, type SubAgentProgressEvent } from '../ai/agents/subagent-presentation.js';
...
function fitStyledLine(text: string, columns: number): string {
...
export class SubAgentNoticeQueue {
  ...
  push(block: string): void { if (block) this.pending.push(block); }

  flush(canWrite: boolean, write: (block: string) => void): void {
    if (!canWrite || this.pending.length === 0) return;
...
export class MultiAgentProgressView {
  private readonly agents = new Map<string, MultiAgentSnapshot>();
  private readonly aliases = new Map<string, number>();
  private readonly runs = new Map<string, SubAgentProgressEvent>();

  private alias(id: string, locale: UiLocale): string {
    if (!this.aliases.has(id)) this.aliases.set(id, this.aliases.size);
  ...
  updateRun(event: SubAgentProgressEvent, columns = 100, locale: UiLocale = 'zh-CN'): string {
    const previous = this.runs.get(event.agentId);
    if (previous && (previous.turn > event.turn || (previous.turn === event.turn
      && (previous.kind === 'finished' || event.timestamp < previous.timestamp)))) return '';
    ...
    if (event.kind === 'activity' || (event.kind === 'started' && previous?.turn === event.turn)) return '';
    ...
    if (event.kind === 'finished') {
      ...
      if (event.resultSummary) text += line(`${labels.result}：${event.resultSummary}`);
    }
    return text;
  }

  update(event: MultiAgentEvent): void {
    if (!event.agent.parentId) return;
    ...
    if (this.agents.size > 32) {
      ...
      if (retired) this.agents.delete(retired.id);
    }
  }

  summary(now = Date.now(), columns = 100, locale: UiLocale = 'zh-CN'): string {
    const labels = getUiCopy(locale).subAgents;
