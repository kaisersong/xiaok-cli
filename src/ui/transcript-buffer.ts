...
export type TranscriptBufferEntry =
  ...
  | { kind: 'tool_use'; agentId: string; name: string; summary: string }
  | { kind: 'tool_result'; agentId: string; name: string; content: string; isError: boolean }
  | { kind: 'image'; mediaType: string; width?: number; height?: number }
  | { kind: 'command_output'; command: string; output: string }
...
function annotateTruncation(text: string, limit: number): string {
  const originalBytes = Buffer.byteLength(text, 'utf8');
  if (originalBytes <= limit) return text;
  const kb = Math.round(originalBytes / 1024);
  return `${truncateToBytes(text, limit)}\n… [truncated, original ${kb} KB]`;
}

function entryBytes(entry: TranscriptBufferEntry): number {
  switch (entry.kind) {
    case 'user':
    case 'assistant':
    case 'thinking':
...
export class TranscriptBuffer {
  private entries: TranscriptBufferEntry[] = [];
  private totalBytes = 0;
  private readonly entryByteLimit: number;
  ...
  private cap(entry: TranscriptBufferEntry): TranscriptBufferEntry {
    switch (entry.kind) {
      case 'user':
      case 'assistant':
      case 'thinking':
  ...
  private evictIfNeeded(): void {
    if (this.totalBytes <= this.sessionByteLimit) return;

    for (let index = 0; index < this.entries.length - 1; index += 1) {
      if (this.totalBytes <= this.sessionByteLimit) return;
      const entry = this.entries[index];
      if (entry.kind !== 'tool_result') continue;
      if (entry.content.startsWith('… [evicted')) continue;

      const before = entryBytes(entry);
      const evicted: TranscriptBufferEntry = {
        kind: 'tool_result',
        agentId: entry.agentId,
        name: entry.name,
        isError: entry.isError,
...
export function recordToolObservation(
  ...
  event: { agentId: string; toolName: string; result: string; ok: boolean },
): void {
  buffer.record({
    kind: 'tool_result',
...
export function renderTranscriptText(entries: TranscriptBufferEntry[]): string {
  if (entries.length === 0) {
  ...
  for (const entry of entries) {
    switch (entry.kind) {
      ...
      case 'command_output':
        ...
        break;
      case 'image':
        lines.push(formatRailLine(formatImagePlaceholder(
