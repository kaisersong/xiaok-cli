...
if(process.argv[2]==='--room-discussion-v1'){
  ...
  try{await (await import('./commands/room-discussion.js')).runRoomDiscussionCli(process.argv.slice(3));}
  catch(error){process.stderr.write(`${error instanceof Error?error.message:'room_discussion_failed'}\n`);process.exitCode=1;}
}else{
