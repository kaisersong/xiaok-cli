...
export function validateHarnessManifest(input: unknown): { ok: true } | { ok: false; errors: string[] } {
  const errors: string[] = [];
  ...
  if (!Array.isArray(manifest.tests) || manifest.tests.length === 0) errors.push('tests');
  if (typeof manifest.rollback !== 'string' || manifest.rollback.trim().length === 0) errors.push('rollback');
  ...
  if (typeof reviews.internal !== 'string' || reviews.internal.trim().length === 0) errors.push('reviews.internal');
  if (typeof reviews.qodercli !== 'string' || reviews.qodercli.trim().length === 0) errors.push('reviews.qodercli');
