...
function isSalesDeckOutputPrompt(prompt: string): boolean {
...
function isDeckSourceObject(beforeDeck: string, afterDeck: string): boolean {
...
function buildGenericUnderstanding(input: BuildTaskUnderstandingInput): TaskUnderstanding {
  return {
    goal: input.prompt.trim(),
    deliverable: '任务结果',
    taskType: 'unknown',
