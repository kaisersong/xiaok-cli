export type RuntimeEvent =
  ...
  | { type: 'artifact_recorded'; sessionId: string; turnId: string; intentId: string; stageId: string; artifactId: string; label: string; kind: string; path?: string; creator?: string; mimeType?: string }
  ...
  | { type: 'post_tool_use_failure'; sessionId: string; turnId: string; toolName: string; toolInput: Record<string, unknown>; toolUseId: string; error: string; isInterrupt?: boolean }
  | { type: 'permission_request'; sessionId: string; turnId: string; toolName: string; toolInput: Record<string, unknown> }
  | { type: 'permission_denied'; sessionId: string; turnId: string; toolName: string; toolInput: Record<string, unknown>; toolUseId: string; reason: string }
  | { type: 'notification'; sessionId: string; message: string; title?: string; notificationType: string }
  | { type: 'user_prompt_submit'; sessionId: string; prompt: string }
  | { type: 'session_start'; sessionId: string; source: 'startup' | 'resume' | 'clear' | 'compact'; agentType?: string; model?: string }
  | { type: 'session_end'; sessionId: string; reason: string }
  | { type: 'stop'; sessionId: string; stopHookActive: boolean; lastAssistantMessage?: string }
  | { type: 'stop_failure'; sessionId: string; error: string; errorDetails?: string; lastAssistantMessage?: string }
  | { type: 'subagent_start'; sessionId: string; agentId: string; agentType: string }
  | { type: 'subagent_stop'; sessionId: string; agentId: string; agentType: string; stopHookActive: boolean; agentTranscriptPath: string; lastAssistantMessage?: string }
