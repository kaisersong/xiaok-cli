...
import { validateArtifactBytes, type StructuralKind, type StructuralValidationResult } from '../../quality/artifact-structure.js';
import { completionEvidenceFlow, type CompletionEvidenceInput, type EvidenceEffect, type EvidenceEffectResult, type EvidenceValidationResult } from './completion-evidence.js';
...
export function throwEvidenceAbort(signal: AbortSignal): void {
  if (signal.aborted) throw signal.reason ?? new DOMException('Aborted', 'AbortError');
}
export function assertEvidenceActive(options: EvidenceIoOptions): void {
  throwEvidenceAbort(options.signal);
  options.assertActive?.();
  throwEvidenceAbort(options.signal);
}

export async function validateCompletionEvidenceAsync(input: CompletionEvidenceInput, options: EvidenceIoOptions): Promise<EvidenceValidationResult> {
  ...
  while (!step.done) {
    ...
    try { value = await executeEffect(step.value, options); }
    catch (error) {
      // Cancellation must not enter the ordinary missing-file/fail-open catches.
      assertEvidenceActive(options);
      step = flow.throw(error); continue;
    }
    assertEvidenceActive(options);
...
async function executeEffect(effect: EvidenceEffect, options: EvidenceIoOptions): Promise<EvidenceEffectResult> {
  switch (effect.kind) {
    case 'exists':
      try { await raw(access(effect.path), options); return true; }
      catch { assertEvidenceActive(options); return false; }
...
function raw<T>(promise: Promise<T>, options: EvidenceIoOptions): Promise<T> {
  options.trackPending(promise);
  return promise;
}

async function validateStructureAsync(path: string, kind: StructuralKind, options: EvidenceIoOptions): Promise<StructuralValidationResult> {
  try {
    assertEvidenceActive(options);
