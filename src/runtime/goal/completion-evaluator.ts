...
export class GoalCompletionEvaluator {
  evaluate(goal: GoalState, evidence: GoalEvidenceEnvelope[]): GoalCompletionEvaluation {
    ...
    const missingKinds = goal.expectedEvidenceKinds.filter(kind =>
      !current.some(item => item.record.kind === kind && isStrictlyValid(item)),
    );
...
function isStrictlyValid(envelope: GoalEvidenceEnvelope): boolean {
  const record = envelope.record;
  if (!record.summary.trim()) return false;
  switch (record.kind) {
    case 'answer':
    ...
    case 'command_action': {
      ...
      return Array.isArray(commands) && commands.length > 0 && commands.every(command =>
        ...
        && command !== null
        && (command as Record<string, unknown>).exitCode === 0,
      );
    }
    case 'project_update':
