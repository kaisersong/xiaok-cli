import type { TraceBundleV1, TraceEvent, TraceTask } from '../trace/schema.js';
import type { DiagnosisAction, DiagnosisEvidence, DiagnosisFinding, DiagnosisHealth, DiagnosisReport } from './types.js';

export function diagnoseTraceBundle(bundle: TraceBundleV1): DiagnosisReport {
  const findings: DiagnosisFinding[] = [];
  const evidence: DiagnosisEvidence[] = [];
  const actions = new Map<string, DiagnosisAction>();
...
export function choosePrimaryFinding(findings: DiagnosisFinding[]): DiagnosisFinding | null {
  const severityScore: Record<DiagnosisFinding['severity'], number> = {
    critical: 4,
    high: 3,
    medium: 2,
    low: 1,
...
function inferHealth(findings: DiagnosisFinding[], bundle: TraceBundleV1): DiagnosisHealth {
  if (findings.some((finding) => finding.category === 'blocked_task')) return 'blocked';
  if (findings.some((finding) => finding.category === 'empty_artifact')) return 'failed';
  if (findings.some((finding) => finding.category === 'approval_wait')) return 'waiting';
  if (bundle.tasks.some((task) => task.status === 'in_progress' || task.status === 'accepted')) return 'running';
  return findings.length > 0 ? 'unknown' : 'healthy';
}

function taskEvidence(task: TraceTask): DiagnosisEvidence {
  return {
    id: `task:${task.id}`,
    label: task.title,
    data: {
...
function findUnresolvedApproval(events: TraceEvent[]): TraceEvent | null {
