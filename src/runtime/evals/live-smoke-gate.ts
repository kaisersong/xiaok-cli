...
export function createDefaultAheLiveSmokeChecks(): AheLiveSmokeCheck[] {
  return [
    {
      id: 'tmux',
      label: 'tmux TTY e2e',
...
export async function runAheLiveSmokeGate(input: {
  ...
  const results = input.skipReason
    ? input.checks.map((check): AheLiveSmokeResult => ({
        id: check.id,
        label: check.label,
  ...
  const summary: AheLiveSmokeSummary = {
    schemaVersion: 1,
...
async function runCheck(check: AheLiveSmokeCheck): Promise<AheLiveSmokeResult> {
...
function classifyRecommendation(results: AheLiveSmokeResult[], skipReason?: string): AheLiveSmokeSummary['recommendation'] {
...
function classifyResult(result: AheLiveSmokeCommandResult): AheLiveFailureClass {
...
function runCommand(command: string[], timeoutMs: number): Promise<AheLiveSmokeCommandResult> {
