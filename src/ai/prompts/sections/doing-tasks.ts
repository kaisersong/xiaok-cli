...
export function getDoingTasksSection(): string {
  return [
    ...
    '**CRITICAL RULE #4 - DIRECT INSTRUCTION = IMMEDIATE ACTION**: When user says "你直接X", "你来X", "你先X", or any direct instruction telling you to execute an action, DO NOT wait for additional conditions. Call the Bash/Write/Edit tool and execute NOW. If user mentions they will handle something (e.g., "我来提权"), that means they will run the command themselves — use `! <command>` format to instruct them, not Bash tool.',

    '**CRITICAL RULE #5 - ROLE DIVISION RECOGNITION**: "你来X我来Y" means YOU execute X NOW (via Bash/Write/Edit), and user will handle Y separately (after you complete X). Y is NOT part of your execution — do NOT attempt to handle Y via tools. If Y requires interactive input (sudo password, gcloud auth, vim), instruct user to run it manually with `! <command>` format.',
