...
import type {
  IntentBoundaryConfig,
  IntentBoundaryDebugEvent,
  IntentBoundaryInput,
  LlmBoundaryDecision,
  RuleBoundaryDecision,
...
export interface IntentBoundaryResolverOptions {
  ...
  llmClassify?: (
    input: IntentBoundaryInput,
    ruleDecision: Extract<RuleBoundaryDecision, { kind: 'ambiguous' }>,
  ) => Promise<LlmBoundaryDecision>;
  emitDebug?: (event: IntentBoundaryDebugEvent) => void;
}

export function createIntentBoundaryResolver(options: IntentBoundaryResolverOptions): {
  resolve(input: IntentBoundaryInput): Promise<ValidatedBoundaryDecision>;
} {
  let clarificationCount = 0;

  async function resolve(input: IntentBoundaryInput): Promise<ValidatedBoundaryDecision> {
    const ruleDecision = classifyIntentBoundaryByRules(input);

    if (ruleDecision.kind === 'definite_non_intent') {
      const result = { kind: 'non_intent', reason: ruleDecision.reason, source: 'rule' } as const;
  ...
  function applyClarificationLimit(result: ValidatedBoundaryDecision): ValidatedBoundaryDecision {
...
function resolveFallback(
  config: IntentBoundaryConfig,
  legacy: ValidatedBoundaryDecision,
  ruleDecision: Extract<RuleBoundaryDecision, { kind: 'ambiguous' }>,
): ValidatedBoundaryDecision {
  ...
  if (legacy.kind === 'intent') return legacy;
...
function emit(
  options: IntentBoundaryResolverOptions,
  result: ValidatedBoundaryDecision,
  reason: string,
): void {
  options.emitDebug?.({
    type: 'intent_boundary_decision',
...
function mapLlmKind(kind: LlmBoundaryDecision['kind']): ValidatedBoundaryDecision['kind'] {
  if (kind === 'create_intent') return 'intent';
  if (kind === 'ask_clarification') return 'clarify';
  return 'non_intent';
}
