...
export type RuleBoundaryDecision =
  | { kind: 'definite_non_intent'; reason: string; plannerHint?: PlannerHint }
  | { kind: 'definite_intent'; reason: string; plannerHint: PlannerHint }
  | { kind: 'ambiguous'; ambiguityType: AmbiguityType; reason: string; plannerHint?: PlannerHint };
...
export type LlmBoundaryDecision =
  | { kind: 'answer_directly'; confidence: number; reason: string }
  | {
      kind: 'create_intent';
      confidence: number;
      intentType: IntentType;
  ...
  | { kind: 'ask_clarification'; confidence: number; question: string; reason: string };

export type ValidatedBoundaryDecision =
  | { kind: 'non_intent'; reason: string; source: BoundarySource }
  | { kind: 'intent'; plan: IntentPlanDraft; source: BoundarySource }
  | { kind: 'clarify'; question: string; reason: string; source: BoundarySource };
