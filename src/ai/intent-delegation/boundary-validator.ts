...
type RuleIntentDecision = { kind: 'rule_intent'; reason: string };
...
export function validateBoundaryDecision(
  input: IntentBoundaryInput,
  candidate: ValidateBoundaryCandidate,
  ...
  if (candidate.decision.kind === 'answer_directly') {
    if (
      candidate.ruleDecision?.kind === 'ambiguous'
  ...
  if (
    candidate.decision.kind === 'create_intent'
...
function isSupportedIntentType(value: unknown): value is IntentType {
  return typeof value === 'string' && SUPPORTED_INTENT_TYPES.has(value);
}

function isConcreteDeliverable(value: string): boolean {
  const normalized = value.trim().toLowerCase();
  if (!normalized || VAGUE_DELIVERABLE_TERMS.has(normalized)) return false;
  return /(报告|幻灯片|ppt|slides|文档|表格|清单|方案|备忘录|memo|markdown|md|代码|测试|summary|report|deck)/iu.test(normalized);
}

function buildDeliverableInstruction(intentType: IntentType, deliverables: string[]): string {
