...
function variantMetadataKey(variant: ProviderModelVariant): string {
...
const METADATA_FIELDS = ['capabilities', 'runtimeOptions', 'runtimeConstraints'] as const;

/**
 * 指出重复 wireModel 的哪些字段真正不一致。没有这个，守卫在 CI 变红后
 * 还要人工逐行 diff registry 才能找到漏改的那一处。
 */
function divergentMetadataFields(variants: ProviderModelVariant[]): string[] {
  return METADATA_FIELDS.filter((field) => {
    const rendered = new Set(variants.map((variant) => {
      const parsed = JSON.parse(variantMetadataKey(variant)) as Record<string, unknown>;
...
export function resolveProviderModelVariant(
  ...
  if (metadataKeys.size > 1) {
    ...
    throw Object.assign(
      new Error(
        `Ambiguous model variants for ${profile.id}/${wireModel}: `
        + `${ids} disagree on ${fields}`,
      ),
      { code: 'MODEL_VARIANT_AMBIGUOUS' as const },
    );
  }
...
export function findCatalogModel(
  ...
  const exact = variants.find(
    (variant) => variant.modelId === modelId && variant.model === wireModel,
  );
  if (exact) return exact;

  const byWire = variants.filter((variant) => variant.model === wireModel);
  if (byWire.length === 0) return undefined;
  // 元数据不一致时无法确定该采用哪一条，退化为未命中。
  if (new Set(byWire.map(variantMetadataKey)).size > 1) return undefined;
