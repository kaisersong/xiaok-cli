...
// We keep request shape minimal and stable; tests pin behaviour with mocked fetch
// rather than hitting the live API. include_answer is intentionally disabled —
// callers that want a synthesized answer can run a follow-up tool call.
...
export function createTavilySearchProvider(options: TavilyOptions): SearchProvider {
  if (!options.apiKey) {
  ...
  return {
    ...
    async search(input: SearchRunInput): Promise<SearchHit[]> {
      ...
      try {
        parsed = (await response.json()) as TavilyResponseBody;
      } catch (error) {
        throw new SearchProviderError(`invalid json: ${toMessage(error)}`, { kind: 'parse' });
      }

      const rawResults: TavilyRawResult[] = Array.isArray(parsed.results)
        ? (parsed.results as TavilyRawResult[])
        : [];
