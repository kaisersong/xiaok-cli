...
const PROMPT_DOC_NAMES = ['AGENTS.md', 'CLAUDE.md'] as const;
...
const DEFAULT_GIT_PROVIDER: GitContextProvider = {
  getBranch: getCurrentBranch,
  isDirty: isGitDirty,
  getRecentCommits: getRecentCommitSubjects,
};

function collectSearchDirs(cwd: string): string[] {
  const dirs: string[] = [];
...
export async function loadAutoContext(options: LoadAutoContextOptions): Promise<LoadedContext> {
  const maxChars = options.maxChars ?? 8_000;
  const docs: LoadedContextDoc[] = [];
...
export function formatLoadedContext(context: LoadedContext): string {
