...
type CanonicalPlan =
  ...
  | { kind: 'array'; values: CanonicalPlan[] }
  | { kind: 'object'; entries: Array<{ keyToken: string; value: CanonicalPlan }> };
...
function addUtf16(state: ValidationState, count: number): void {
  state.utf16CodeUnits += count;
  if (
    state.utf16CodeUnits
...
function addNode(state: ValidationState, depth: number): void {
...
function assertNotAncestor(state: ValidationState, value: object): void {
...
function validateArray(
  ...
  for (let index = 0; index < keys.length; index += 1) {
    ...
    if (key !== 'length') {
      ...
      if (
        !capturedNumberIsInteger(numericIndex)
        || numericIndex < 0
        || numericIndex >= length
  ...
  const values = new CapturedArray<CanonicalPlan>(length);
...
function validateObject(
  ...
  const keys = new CapturedArray<string>(ownKeys.length);
  ...
  const entries = new CapturedArray<{
    keyToken: string;
    value: CanonicalPlan;
  }>(keys.length);
  state.ancestors[state.ancestors.length] = value;
...
function validateValue(
  ...
  return validateObject(value as Record<string, unknown>, depth, state);
}

function emit(plan: CanonicalPlan): string {
  if (plan.kind === 'token') {
    return plan.token;
  }
  if (plan.kind === 'array') {
