...
async function runChat(initialInput: string | undefined, opts: ChatOptions): Promise<void> {
  ...
  const memoryStore: MemoryStore = await createMemoryStoreAsync(
    opts.print || opts.json
      ? { ...((config.memory as Record<string, unknown> | undefined) ?? {}), type: 'file' }
  ...
  let activeModelWatchdog: ReturnType<typeof createTurnActivityWatchdog> | undefined;
  ...
  const runRuntimeTurn = async (
    ...
    try {
      ...
      if (watchdog.didTimeout()) throw new Error('模型长时间无响应，已取消本轮；可以继续输入或切换模型。可通过 XIAOK_TURN_TIMEOUT_MS 调整空闲超时。');
      throw error;
    } finally {
  ...
  const registryFactory = createPlatformRegistryFactory({
    notifyReminder: (message) => {
      const block = `\n[reminder] ${message}\n`;
      if (opts.print || opts.json) { process.stdout.write(block); return; }
    ...
    runInteractiveBash: !opts.print && !opts.json && process.stdin.isTTY && process.stdout.isTTY && process.platform !== 'win32'
      ? async (input, context) => {
        if (interactiveBashActive) return 'Error: 本地交互终端正被另一条命令使用，请稍后重试。';
        ...
        try {
          return await withPausedLiveActivity(async () => {
            try {
              ...
              if (context?.toolInvocationId) context.runtimeFactSink?.emit({ invocationId: context.toolInvocationId, toolName: 'bash', factKind: 'command_result', exitCode: result.timedOut ? null : result.exitCode });
              if (result.timedOut) return `Error: 命令超时\n${result.output}`;
          ...
          try { inputLease.resume(); busy?.resume(); } finally { interactiveBashActive = false; flushSubAgentNotices(); }
        }
      } : undefined,
    platform,
    source: 'chat',
  ...
  suspendInteractiveUi = (
    ...
    try {
      scrollRegion.end();
    } catch (e) { terminalLog.warn('scrollRegion.end failed in suspendInteractiveUi', (e as Error).message) }
  };
  ...
  const withPausedLiveActivity = async <T>(action: () => Promise<T>): Promise<T> => {
    try { return await runtimeState.withPausedLiveActivity(action); }
    finally { flushSubAgentNotices(); }
  };
  ...
  const terminalOutput = createTerminalOutputRouter({
    ...
    onFailure: (stream, error, fallback) => {
      ...
      if (stream === 'stdout' && isBrokenPipeError(error) && fallback === 'stderr') return;
      suspendInteractiveUi(stream + '_stream_error', error, fallback);
    },
  });
  process.stdout.write = ((chunk: any, ...args: any[]) => {
    if (!terminalOutput.hasOutput()) return true;
    const text = typeof chunk === 'string' ? chunk : Buffer.from(chunk).toString('utf8');
    transcriptLogger.recordOutput('stdout', text);
    if (terminalUiSuspended && !terminalUiFallbackStream) return true;
  ...
  try {
    ...
    setStreamErrorHandler((error, stream) => {
      if (stream !== process.stdout && stream !== process.stderr) return false;
    ...
    if (trimmed === '/models') {
      ...
      if (selected) {
        ...
        try {
          ...
          if (currentProfile !== nextProfile) writeCommandOutput(trimmed, '模型协议已切换，历史已转换为最近的可见上下文（最多 40KB）。\n');
          memoryStore.setLLMFn?.(createLLMFromAdapter(adapter));
