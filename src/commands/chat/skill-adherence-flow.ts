import {
  buildComplianceReminder,
  evaluateSkillCompliance,
  type SkillComplianceResult,
} from '../../ai/skills/compliance.js';
import {
  recordSkillEvidence,
...
export function createStrictSkillAdherenceFlow(
  deps: StrictSkillAdherenceFlowDeps,
): StrictSkillAdherenceFlow {
  const setSkillExecutionState = (state: SessionSkillExecutionState): void => {
  ...
  const applyComplianceResult = (
    ...
    setSkillExecutionState(updateSkillCompliance(
      deps.getSkillExecutionState(),
...
function buildComplianceEvidenceView(invocation: SkillInvocationState) {
  return {
...
function formatFailedComplianceProgressNote(compliance: SkillComplianceResult): string {
