...
export async function runPtyCommand(command: string, options: PtyCommandOptions): Promise<{
  output: string; exitCode: number; timedOut: boolean;
}> {
  options.signal?.throwIfAborted();
  if ((options.platform ?? process.platform) === 'win32') throw new Error('Windows sudo 不使用此交互终端；请使用管理员终端。');
  const input = options.input ?? process.stdin;
  if (!input.isTTY) throw new Error('sudo 需要本地交互终端，当前会话不可输入密码。');
  let backend: typeof import('node-pty');
  try { backend = await import('node-pty'); }
  catch (cause) {
    options.signal?.throwIfAborted();
    const code = cause && typeof cause === 'object' && 'code' in cause ? String(cause.code) : '';
    const detail = cause instanceof Error ? `${cause.name}: ${cause.message}` : String(cause);
    let modulePath = 'not resolved';
    try { modulePath = createRequire(import.meta.url).resolve('node-pty'); } catch { /* optional dependency */ }
    ...
    throw new Error(
      `交互终端组件 node-pty 加载失败，命令尚未执行。请检查可选依赖是否安装及原生模块是否兼容当前 Node。\n`
  ...
  const stop = () => {
    ...
    escalation = setTimeout(() => {
      if (exited) return;
      try { process.kill(-proc.pid, 'SIGKILL'); } catch { /* onExit remains the completion boundary */ }
      try { proc.kill('SIGKILL'); } catch { /* do not report a live process as complete */ }
    }, 1000);
  };
  const onAbort = () => { interrupted = true; stop(); };
  const onInput = (data: Buffer | string) => {
    if (!ready || exited || interrupted || timedOut) return;
    const text = data.toString();
    if (text === '\x03' || text === '\x1b') { onAbort(); return; }
    proc.write(text);
  };
  const onResize = () => {
    if (!exited) { try { proc.resize(process.stdout.columns || 80, process.stdout.rows || 24); } catch { /* exiting */ } }
  };
  ...
  try {
    return await new Promise((resolve, reject) => {
      exitSubscription = proc.onExit(({ exitCode }) => {
        exited = true;
        if (!ready) output = startup.slice(0, limit);
        if (interrupted) reject(options.signal?.reason ?? new DOMException('Aborted', 'AbortError'));
        else resolve({ output, exitCode, timedOut });
      });
      dataSubscription = proc.onData(data => {
        if (!ready) {
          ...
          if (index < 0) { startup = startup.slice(-4096); return; }
          data = startup.slice(index + marker.length);
          startup = ''; ready = true;
          input.setRawMode(true);
          input.on('data', onInput); input.resume();
        }
        output += data.slice(0, Math.max(0, limit - output.length));
        try { options.write(data); } catch { onAbort(); }
      });
