...
import { DEFAULT_CONFIG, type Config, type ModelAdapter } from '../types.js';
...
export async function runRoomDiscussion(input: {
  prompt: string; config: Config; signal?: AbortSignal;
  createAdapter?: (config: Config) => ModelAdapter;
}): Promise<string> {
  if(typeof input.prompt!=='string'||!input.prompt.trim())throw new Error('room_discussion_input_invalid');
  ...
  try{
    ...
    for await(const chunk of streamStatelessSideCallProviderConversation({
      ...
      if(chunk.type==='tool_use')throw new Error('room_discussion_tool_denied');
  ...
  finally{input.signal?.removeEventListener('abort',forwardAbort);}
}

export async function runRoomDiscussionCli(args: string[]): Promise<void> {
  if(args.length===1&&args[0]==='--probe'){process.stdout.write(JSON.stringify(ROOM_DISCUSSION_PROTOCOL)+'\n');return;}
  if(args.length)throw new Error('room_discussion_arguments_invalid');
  let raw='';
  for await(const chunk of process.stdin){raw+=chunk;if(Buffer.byteLength(raw)>1024*1024)throw new Error('room_discussion_input_too_large');}
  const request=JSON.parse(raw) as {prompt?:unknown};
  if(typeof request.prompt!=='string')throw new Error('room_discussion_input_invalid');
  ...
  let config:Config;
  try{config=normalizeConfig(JSON.parse(await readFile(getConfigPath(),'utf8')));}
  catch(error){if((error as NodeJS.ErrnoException).code!=='ENOENT')throw error;config=DEFAULT_CONFIG;}
  const controller=new AbortController();
  const abort=()=>controller.abort(new Error('room_discussion_cancelled'));
  process.once('SIGTERM',abort);process.once('SIGINT',abort);
  try{process.stdout.write(JSON.stringify({protocol:'room_discussion_v1',text:await runRoomDiscussion({prompt:request.prompt,config,signal:controller.signal})})+'\n');}
