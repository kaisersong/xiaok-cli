...
import { buildProjectTraceBundleFromKSwarmDetail, buildSessionTraceBundleFromSnapshots, loadTaskSnapshotsForSession, writeTraceBundleToPath, } from '../../src/runtime/trace/exporter.js';
...
import { createEmptySessionIntentLedger, cloneSessionIntentLedger, createIntentLedgerRecord } from '../../src/runtime/intent-delegation/types.js';
...
import { buildOfficialInstallerExecution, getPluginDependencyStatus, } from './plugin-dependency-service.js';
...
import { isComputerUseAutoConnectEligibleApp, loadComputerUsePreference, saveComputerUsePreference, } from './computer-use-capability-service.js';
...
import { createKSwarmGetDynamicWorkflowStatusTool, createKSwarmRunDynamicWorkflowScriptTool, isResumableWorkflowRunStatus, restoreWorkflowScriptBackgroundJob, } from './kswarm-dynamic-workflow-script-tool.js';
...
let _LayeredMemoryStoreClass = null;
let _resolveLayeredConfigFn = null;
try {
    ...
    console.warn('[memory] Could not load layered-store module:', e.message);
}
// ---- Shared memory store (singleton per dataRoot) ----
let _desktopMemoryStore = null;
let _desktopMemoryStoreDataRoot = null;
/**
 ...
 */
function createFallbackMemoryStore(dataRoot) {
    ...
    const store = {
        ...
        clearAll() {
            for (const m of userStore.list())
                userStore.delete(m.id);
        },
    };
    return store;
}
export function getDesktopMemoryStore(dataRoot) {
    ...
    try {
        ...
        console.warn('[memory] LayeredMemoryStore unavailable (native module issue), using fallback:', err.message);
...
async function appendExecRecord(dataRoot, record) {
    try {
        ...
        records.push(record);
        if (records.length > MAX_EXEC_RECORDS)
            records.splice(0, records.length - MAX_EXEC_RECORDS);
        mkdirSync(dirname(filePath), { recursive: true });
        await writeFileAsync(filePath, JSON.stringify(records));
    }
    catch { /* silent */ }
}
async function loadExecRecords(dataRoot) {
    try {
...
function aggregateStats(records) {
    ...
    for (const r of records) {
        ...
        for (const name of names) {
            ...
            if (r.startTime < entry.firstAt)
                entry.firstAt = r.startTime;
            if (r.endTime > entry.lastAt)
                entry.lastAt = r.endTime;
...
export function createTimedActionTools(service, timezone = Intl.DateTimeFormat().resolvedOptions().timeZone) {
    return [
        {
            permission: 'safe',
            definition: {
        ...
        },
        {
            ...
            async execute(input) {
                ...
                if (!name || !prompt)
                    return 'Error: name 和 prompt 不能为空';
                let trigger;
                try {
...
export function createDesktopServices(options) {
    ...
    const initialPlanBootstrapQueue = new KSwarmInitialPlanBootstrapQueue(initialPlanBootstrapStore, input => bootstrapKSwarmInitialPlan(input), { now: options.now });
    const kswarmCreateProjectToolOptions = {
    ...
    let computerUseUnavailableError = computerUsePreference.lastFailureCode === 'COMPUTER_USE_DISABLED_BY_USER'
    ...
    const findPluginDependency = (pluginName, dependencyId) => pluginDependencies.find((entry) => entry.pluginName === pluginName && entry.dependency.id === dependencyId);
    const findPluginDependencyForMcpServer = (pluginName, serverName) => pluginDependencies.find((entry) => entry.pluginName === pluginName && entry.dependency.mcp?.serverName === serverName);
    const isPluginInstalled = (pluginName) => existsSync(join(pluginRootDir, pluginName, 'plugin.json'));
    const getDependencyStatusView = async (entry) => ({
    ...
    const resolvePluginMcpLaunch = async (pluginName, serverName, fallbackCommand, fallbackArgs) => {
    ...
    async function runDependencyCommand(command, args, timeout = 300_000) {
    ...
    const persistComputerUsePreference = () => {
        try {
    ...
    const recordComputerUseReady = (source) => {
        const { lastFailureCode: _lastFailureCode, autoConnectSuspendedReason: _autoConnectSuspendedReason, ...preferenceWithoutFailure } = computerUsePreference;
    ...
    const recordComputerUseFailure = (code) => {
    ...
    const disposePluginMcpServers = (predicate = () => true) => {
    ...
    function markComputerUseRecoverableFailure(error) {
        ...
        recordComputerUseFailure(error.code);
    ...
    const reconnectPluginMcpServers = async (options = {}) => {
        const matchesTarget = (server) => !options.targetServerName || server.name === options.targetServerName;
            ...
            for (const plugin of plugins) {
                ...
                for (const server of plugin.mcpServers) {
                    if (server.type !== 'stdio')
                        continue;
                    if (!matchesTarget({ name: server.name, pluginName: plugin.name }))
                        continue;
                    ...
                    try {
                        ...
                        const baseEnv = 'env' in server ? server.env : undefined;
                        ...
                        if (server.name === 'cua-driver') {
                            ...
                            mcpTools = buildMcpRuntimeTools({ name: server.name, command: server.command }, { listTools: () => Promise.resolve(schemas), callTool: (name, input) => client.callTool(name, input), dispose: () => { transport.dispose(); proc.dispose(); } }, schemas);
                        ...
                        if (server.name === 'cua-driver') {
                            ...
                            if (options.userInitiated || options.autoConnectComputerUse) {
                                recordComputerUseFailure(computerUseUnavailableError.code);
                            }
                        }
    ...
    const getComputerUseCapabilityStatus = () => {
    ...
    const createKSwarmTaskHost = (workspaceRoot) => {
        if (options.runner)
            return host;
    ...
    async function runKSwarmHandoffTask({ handoff, targetParticipantId }) {
        ...
        while (Date.now() < deadline) {
            ...
            if (recovered.snapshot.status === 'completed') {
                ...
                const eventArtifacts = artifactEvents.map(event => ({
                    path: event.filePath || event.path,
                    kind: inferKSwarmArtifactKind(event.kind, event.label),
                    label: event.label,
    ...
    async function runKSwarmReadinessProbe({ targetParticipantId } = {}) {
        ...
        try {
            ...
            if (!config.defaultProvider || !config.defaultModelId || !model || !provider) {
                return { ...base, ok: false, reason: 'model_config_missing' };
            }
            ...
            for (const requiredTool of ['create_project', 'inspect_project', 'continue_project', 'repair_project_task_from_file']) {
                if (!toolNames.has(requiredTool)) {
                    return { ...base, ok: false, reason: 'kswarm_tools_unavailable' };
                }
            }
    ...
    async function runKSwarmWorkflowNode({ handoff, targetParticipantId }) {
        ...
        try {
            ...
            // LLM returned non-structured markdown. Fall back to the raw summary so the
            // node still produces output instead of blocking the whole workflow.
            parsed = {};
        }
    ...
    async function runKSwarmAssignPo({ payload, targetParticipantId }) {
        try {
            const projectId = readString(payload.projectId) || readString(payload.taskId);
            if (!projectId)
                return { ok: false, error: 'project_id_missing' };
    ...
    async function bootstrapKSwarmInitialPlan(input) {
    ...
    async function runKSwarmReviewSubmission({ payload, targetParticipantId }) {
        try {
            const projectId = readString(payload.projectId);
            const taskId = readString(payload.taskId);
            if (!projectId || !taskId)
                return { ok: false, error: 'project_or_task_id_missing' };
            const fromAgent = targetParticipantId || readString(payload.poAgent) || XIAOK_PO_SEED_ID;
    ...
    async function runKSwarmPlanApproved({ payload, targetParticipantId }) {
        try {
            const projectId = readString(payload.projectId) || readString(payload.taskId);
            if (!projectId)
                return { ok: false, error: 'project_id_missing' };
    ...
    return {
        registerTimedActionService(service) {
        ...
        async restartPluginMcpServer(input) {
        ...
        async disableComputerUse() {
            ...
            for (let index = pluginMcpServers.length - 1; index >= 0; index -= 1) {
                if (pluginMcpServers[index].name === 'cua-driver')
                    pluginMcpServers.splice(index, 1);
            }
            computerUseBackend = null;
        ...
        async installPluginDependency(input) {
            const entry = findPluginDependency(input.pluginName, input.dependencyId);
            if (!entry)
                return { success: false, error: 'plugin_dependency_not_found' };
            try {
                ...
                if (!res.ok)
                    return { success: false, error: `installer_download_failed_${res.status}` };
        ...
        async updatePluginDependency(input) {
            const entry = findPluginDependency(input.pluginName, input.dependencyId);
            if (!entry)
                return { success: false, error: 'plugin_dependency_not_found' };
            const dependency = entry.dependency;
            if (!dependency.update)
                return { success: false, error: 'plugin_dependency_update_not_available' };
            if (dependency.update.requiresUserConfirmation && !input.confirmed) {
                return { success: false, error: 'confirmation_required' };
            }
            const status = await getDependencyStatusView(entry);
            if (!status.resolvedBinary)
                return { success: false, error: 'plugin_dependency_binary_missing' };
            const result = await runDependencyCommand(status.resolvedBinary, dependency.update.args ?? []);
            if (!result.success)
                return { success: false, error: result.error };
            return { success: true, status: await getDependencyStatusView(entry) };
        },
        async diagnosePluginDependency(input) {
            const entry = findPluginDependency(input.pluginName, input.dependencyId);
            if (!entry)
                return { success: false, error: 'plugin_dependency_not_found' };
            const status = await getDependencyStatusView(entry);
            ...
            if (!doctor)
                return { success: false, error: 'plugin_dependency_diagnose_not_available' };
            const [, ...args] = doctor;
        ...
        async saveSkillDebugConfig(input) {
            const config = await loadConfig();
        ...
        async testProviderConnection(input) {
            ...
            try {
                ...
                const testMessages = [{ role: 'user', content: [{ type: 'text', text: 'ping' }] }];
        ...
        async listAvailableModelsForProvider(providerId) {
        ...
        async openArtifact(_artifactId) {
        ...
        async updateChannel(id, input) {
            ...
            if (input.webhookUrl !== undefined)
                ch.sendMsgUrl = input.webhookUrl;
            if (input.enabled !== undefined)
                ch.enabled = input.enabled;
            await saveConfig(config);
            return {
                id, type: id, name: ch.name || id,
                webhookUrl: ch.sendMsgUrl,
        ...
        async createMCPInstall(input) {
            const path = join(options.dataRoot, 'mcp-installs.json');
            const installs = await loadJsonFile(path, []);
            const record = {
        ...
        async updateMCPInstall(id, input) {
            const path = join(options.dataRoot, 'mcp-installs.json');
            const installs = await loadJsonFile(path, []);
            const idx = installs.findIndex(c => c.id === id);
            if (idx < 0)
                throw new Error('MCP install not found');
            if (input.name !== undefined)
                installs[idx].name = input.name;
            if (input.source !== undefined)
                installs[idx].source = input.source;
            if (input.command !== undefined)
                installs[idx].command = input.command;
            if (input.enabled !== undefined)
                installs[idx].enabled = input.enabled;
            await saveJsonFile(path, installs);
            return toMCPView(installs[idx]);
        },
        async deleteMCPInstall(id) {
            const path = join(options.dataRoot, 'mcp-installs.json');
            const installs = await loadJsonFile(path, []);
            await saveJsonFile(path, installs.filter(c => c.id !== id));
        },
        async exportTraceBundle(input) {
            try {
        ...
        async diagnose(input) {
            if (input.kind === 'project') {
                ...
                return diagnoseProjectSnapshot(detail);
            }
...
function buildComputerUseNeedsEnablementError() {
    return {
...
function buildComputerUseDisabledUnavailableError() {
    return {
        code: 'COMPUTER_USE_DISABLED_BY_USER',
        message: 'Computer Use 已被用户禁用。',
    };
}
function mapComputerUseStartupError(error) {
...
function resolveComputerUseAppIdentity() {
...
function resolveCodeSignatureTeamId(appPath) {
...
function formatKSwarmRequiredOutputs(outputs) {
...
function inferKSwarmArtifactKind(kind, label) {
    const normalized = String(kind || '').toLowerCase();
    const lowerLabel = String(label || '').toLowerCase();
    if ((normalized === 'text' || normalized === 'other') && lowerLabel.endsWith('.md'))
        return 'markdown';
    return normalized || 'file';
}
function shouldRequireKSwarmArtifactEvidence(task) {
    if ((task.requiredOutputs ?? []).length > 0)
        return true;
    if (task.evidenceContract && task.evidenceContract.required === true)
        return true;
    const text = [
        task.title,
        task.brief,
...
function discoverKSwarmArtifactsFromDirectory(input) {
    ...
    for (const entry of readdirSync(artifactsDir, { withFileTypes: true })) {
        if (!entry.isFile() || entry.name.startsWith('.'))
            continue;
        if (isKSwarmSystemArtifactFilename(entry.name))
            continue;
...
function isKSwarmSystemArtifactFilename(filename) {
...
async function runKSwarmRuntimeTextTask(host, prompt, options = {}) {
...
function isRetryableKSwarmRuntimeTaskFailure(reason) {
...
function buildKSwarmWorkflowNodePrompt(handoff, participantId, options = {}) {
...
function mergeKSwarmEvidenceRefs(rawEvidenceRefs, artifacts) {
    const refs = new Set(readStringArray(rawEvidenceRefs));
    for (const artifact of artifacts) {
        if (artifact.path)
            refs.add(`artifact:${artifact.path}`);
    }
    return [...refs];
}
function buildKSwarmAssignPoPrompt(payload, fallbackWorkerId) {
    const members = readStringArray(payload.members);
    return [
        'KSwarm PO 规划任务。',
...
function buildKSwarmReviewPrompt(payload) {
    return [
        'KSwarm PO 验收任务提交。',
...
function buildKSwarmSynthesisPrompt(detail) {
...
function extractKSwarmJsonObject(text) {
...
function normalizeKSwarmPlan(rawPlan, fallbackWorkerId, context = {}) {
...
function softenGeneratedTemporalQuantityCriteria(acceptanceCriteria, context = {}) {
    ...
    if (userExplicitlyRequestedItemCount(userText))
        return text;
    if (!hasGeneratedHardItemCount(text))
        return text;
    if (!hasCurrentPeriodResearchContext(text))
        return text;
    const softened = text.replace(/(包含|列出|整理)?\s*(至少|不少于|不低于)\s*[0-9一二三四五六七八九十百]+\s*条\s*([^，。；;,.]*)/gu, '尽可能完整覆盖已公开的本期相关动态');
...
function normalizeKSwarmReview(rawReview) {
    return {
...
function normalizeKSwarmWorkflowNodeOutput(rawOutput, fallbackSummary) {
    return {
        ...rawOutput,
        summary: readString(rawOutput.summary) || fallbackSummary || 'workflow node completed',
    };
}
function normalizeKSwarmWorkflowReviewDecision(rawDecision) {
...
function shouldSynthesizeKSwarmProject(detail) {
    const record = isRecord(detail) ? detail : {};
    const project = isRecord(record.project) ? record.project : {};
    if (readString(project.status) === 'delivered' || readString(project.status) === 'closed')
        return false;
    const tasks = Array.isArray(record.tasks) ? record.tasks : [];
...
async function requestKSwarmJson(kswarmService, path, init) {
    if (!kswarmService)
        throw new Error('kswarm_service_missing');
...
export async function recoverInterruptedScriptWorkflows(kswarmService) {
    if (!kswarmService)
        return;
    let projects = [];
    try {
...
export async function resumeOneScriptWorkflow(kswarmService, projectId, workflowRunId) {
    if (!kswarmService)
    ...
    let run = null;
    try {
        const payload = await requestKSwarmJson(kswarmService, `/projects/${encodeURIComponent(projectId)}/workflows/${encodeURIComponent(workflowRunId)}`);
        if (isRecord(payload)) {
...
export function resolveToolOutputArtifactPath(toolInput, result, options = {}) {
    return resolveToolOutputArtifactPathWithOptions(toolInput, result, options);
}
export function resolveWriteToolArtifactPath(toolName, toolInput) {
    if (!isToolName(toolName, 'write') || !isRecord(toolInput))
        return null;
    return readString(toolInput.file_path) || null;
}
function resolveToolOutputArtifactPathWithOptions(toolInput, result, options = {}) {
    try {
...
function isToolName(toolName, expected) {
    return typeof toolName === 'string' && toolName.trim().toLowerCase() === expected;
}
export function attachRuntimeToolRequestScope(toolName, input, sessionId) {
    const toolInput = isRecord(input) ? input : {};
    if (!isToolName(toolName, 'create_project'))
        return toolInput;
...
function resolveBashOutputArtifactPath(toolInput, result, options) {
    if (!isToolName(options.toolName, 'bash'))
        return null;
    if (!result.trim() || result.trimStart().startsWith('Error'))
        return null;
...
function rankArtifactPathCandidates(paths) {
    const seen = new Set();
    return paths
...
function scoreArtifactPath(path) {
    let score = 0;
    if (!/(^|\/)(tmp|private\/tmp|var\/folders)\//.test(path))
        score += 10;
    if (/\.(?:pdf|pptx|docx|xlsx)$/iu.test(path))
        score += 5;
    return score;
}
...
function normalizeArtifactPathCandidate(path) {
    return path.trim().replace(/[),.;，。]+$/u, '');
}
function isFreshExistingArtifactPath(filePath, toolStartedAt) {
    try {
...
async function buildDesktopSessionTraceBundle(input, dataRoot, snapshotStore) {
    if (input.kind === 'task') {
...
async function requireTaskSnapshot(snapshotStore, taskId) {
    const snapshot = await snapshotStore.recoverTask(taskId);
    if (!snapshot)
        throw new Error(`task snapshot not found: ${taskId}`);
    return snapshot;
}
async function fetchKSwarmProjectFullDetail(kswarmService, projectId) {
...
function ensureProvider(config, providerId, input) {
    const profile = getProviderProfile(providerId);
...
function ensureDefaultModel(config, providerId) {
    const profile = getProviderProfile(providerId);
    if (profile) {
...
function createModelConfigSnapshot(config) {
    return {
...
function toolNameToLabel(name, input) {
...
export function createReportArtifactTool() {
    return {
        permission: 'write',
        definition: {
        ...
        async execute(input) {
            ...
            if (!irContent.trim())
                return JSON.stringify({ success: false, error: 'ir_content_required' });
            if (!outputPath)
                return JSON.stringify({ success: false, error: 'output_path_required' });
            ...
            const serverBundlePath = join(pluginDir, 'mcp-servers', 'report-renderer', 'dist', 'server.bundle.js');
            const rendererPath = join(getConfigDir('plugins'), 'kai-report-creator', 'mcp-servers', 'report-renderer', 'dist', 'renderer', 'html-builder.js');
...
async function renderReportArtifactViaBundledMcp({ pluginDir, serverBundlePath, irContent, outputPath, theme, }) {
    ...
    try {
        ...
        const raw = await withReportRendererTimeout(client.callTool('render_report', {
...
function isReportRendererStructurallyValid(validation) {
...
function withReportRendererTimeout(promise, operation) {
    let timer;
...
function isAllowedReportArtifactOutputPath(outputPath) {
    const allowedRoots = [
        process.cwd(),
...
export const READ_MATERIAL_TOOL_DEFINITION = {
    name: 'read_material',
...
export function buildMaterialManifestForPrompt(materials) {
...
export async function executeReadMaterialForDesktop(input, options) {
...
async function executeDesktopTaskTool(toolCall, options) {
...
function normalizeReadMaterialLimit(inputLimit, fallback) {
...
function truncateMaterialText(text, maxChars) {
    if (text.length <= maxChars)
        return text;
    return `${text.slice(0, maxChars)}\n...[截断，原文件 ${text.length} 字符]`;
}
function buildReadMaterialMetadata(material) {
    return {
...
function createReadMaterialResult(ok, payload) {
...
function createDesktopModelRunner(dataRoot, materialRegistry) {
    const cwd = process.cwd();
    ...
    return async ({ taskId, sessionId, prompt, materials, signal, history: hostHistory, emitRuntimeEvent }) => {
        ...
        let skillNamesDetected = [];
        let skillTriggerType = 'auto';
        ...
        const messages = [
            ...hostHistory.map((h) => ({ role: h.role, content: [{ type: 'text', text: h.content }] })),
            {
                role: 'user',
                content: userContent,
            }
        ...
        while (iteration < DESKTOP_MODEL_TOOL_LOOP_MAX_ITERATIONS) {
            ...
            if (Date.now() > taskDeadline)
                throw new Error('任务超时（30分钟），可能是网络不稳定或模型响应过慢。请检查网络后重试。');
            iteration++;
            // Budget check
            if (skillInvocation) {
            ...
            for (const toolCall of toolCalls) {
                ...
                if (isToolName(toolCall.name, 'read')) {
                    ...
                    if (skillInvocation) {
                        appendTrace(dataRoot, {
                            ...
                            toolName: 'read_reference', details: String(toolCall.input.file_path || ''),
                        });
                    }
                }
                ...
                if (toolCall.name === 'skill') {
                    try {
                        const extracted = extractSkillNames(toolCall.input);
                        if (skillNamesDetected.length === 0 || skillTriggerType === 'auto') {
                            skillNamesDetected = extracted;
                            if (skillTriggerType === 'auto')
                                skillTriggerType = 'tool_call';
                ...
                if (!isInternalTool) {
                    const label = toolNameToLabel(toolCall.name, toolCall.input);
...
export function createKSwarmCreateProjectTool(kswarmService, options = {}) {
    return {
        permission: 'safe',
        definition: {
            name: 'create_project',
        ...
        async execute(input) {
            const { name, goal, requirements, memberNames = [], memberCount = 0, workFolder, executionMode, _xiaokRequestScope } = input;
            ...
            try {
                ...
                if (!agentsRes.ok)
                    return JSON.stringify({ error: 'Cannot fetch agents from kswarm' });
                const { agents } = await agentsRes.json();
                // 2. 选 PO agent（优先 dedicated xiaok-po，兼容旧 xiaok）
                const poAgent = getPreferredPoAgentId(agents);
                if (!poAgent)
                    return JSON.stringify({ error: 'No agents available. Create an agent in kswarm first.' });
                // 3. 解析智能体需求
                ...
                const explicitlyNamedMemberIds = new Set((Array.isArray(memberNames) ? memberNames : [])
                ...
                if (memberCount > 0) {
                    ...
                    if (canCreate > 0) {
                        const createResults = await Promise.all(Array.from({ length: canCreate }, (_, i) => kswarmService.request('/agents', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                        ...
                        for (const newAgent of createResults) {
                            ...
                            if (newAgentId && !resolvedMembers.includes(newAgentId))
                                resolvedMembers.push(newAgentId);
                        }
                    }
                }
                ...
                if (!res.ok)
                    return JSON.stringify({ error: `Failed to create project: ${res.status}` });
                const { project, reused } = await res.json();
                let planningStatus;
                ...
                return JSON.stringify({ error: `KSwarm service unavailable: ${err.message}` });
            }
        },
    };
}
...
function normalizeCreateProjectExecutionMode(value) {
    if (typeof value !== 'string')
        return undefined;
    const normalized = value.trim().toLowerCase();
    if (normalized === 'workflow' || normalized === 'workflow_preferred')
...
function inferCreateProjectExecutionModeFromIntent(text) {
    if (/(direct|quick|快速编排|直接编排|快速执行|直接执行)/i.test(text))
        return 'direct';
...
function normalizeCreateProjectKeyPart(value) {
    return typeof value === 'string' ? value.trim().replace(/\s+/g, ' ') : '';
}
function buildCreateProjectPlanningGuidanceForTool(input) {
...
function isAnalysisReportLikeProject(text) {
...
export function createKSwarmContinueProjectTool(kswarmService) {
    return {
        permission: 'safe',
        definition: {
        ...
        async execute(input) {
            ...
            try {
                ...
                return JSON.stringify({ ok: false, error: `KSwarm service unavailable: ${err.message}` });
            }
        },
    };
}
...
export function createKSwarmInspectProjectTool(kswarmService) {
    return {
        permission: 'safe',
        definition: {
        ...
        async execute(input) {
            ...
            try {
                const resolved = projectIdInput
                    ? { ok: true, projectId: projectIdInput, match: { mode: 'project_id' } }
...
async function resolveKSwarmProjectByName(kswarmService, projectName) {
    const projectsRes = await kswarmService.request('/projects');
    const payload = await projectsRes.json().catch(() => ({}));
    if (!projectsRes.ok) {
        return { ok: false, error: payload.error || `projects_list_failed:${projectsRes.status}`, status: projectsRes.status };
    }
    ...
    if (sortedMatches.length === 0) {
        return { ok: false, error: 'project_not_found', projectName };
    }
...
function inferArtifactWriteDir(detail) {
...
function compareProjectUpdatedDesc(left, right) {
...
function summarizeKSwarmTask(task) {
    if (!isRecord(task))
        return task;
...
async function readLatestProjectArtifacts(kswarmService, projectId, detail) {
    ...
    for (const artifact of artifacts) {
        ...
        try {
            ...
            results.push({ ...base, readError: String(err.message || err) });
        }
    }
    return results;
}
function collectProjectArtifacts(detail) {
    const collected = [];
    const seen = new Set();
    const add = (artifact, inspectionPriority = 0) => {
        if (!isRecord(artifact))
            return;
    ...
    for (const task of tasks) {
        ...
        if (!interventionTaskIds.has(taskId))
            continue;
        for (const artifact of collectTaskArtifacts(task))
            add(artifact, 1);
    }
    const workspace = isRecord(detail.workspace) ? detail.workspace : null;
...
function getInterventionArtifactTaskIds(detail, tasks) {
    const ids = new Set();
    ...
    if (parentTaskId)
        ids.add(parentTaskId);
    for (const task of tasks) {
        const taskId = String(task.id || '');
        const taskParentId = stringOrUndefined(task.parentTaskId);
        if (taskParentId === primaryTaskId || (parentTaskId && taskParentId === parentTaskId))
            ids.add(taskId);
    }
    return ids;
}
...
function isReadableProjectArtifact(artifact) {
    ...
    return (INSPECT_PROJECT_READABLE_EXTENSIONS.has(ext) ||
        mimeType.startsWith('text/') ||
        mimeType === 'application/json' ||
        mimeType === 'application/csv');
}
function compareArtifactUpdatedDesc(left, right) {
...
function compareArtifactForInspection(left, right) {
...
function summarizeArtifact(artifact, url) {
    return {
...
export function createKSwarmRepairProjectTaskFromFileTool(kswarmService) {
    return {
        permission: 'safe',
        definition: {
...
export function createKSwarmRepairProjectTaskTool(_kswarmService) {
    return {
        permission: 'safe',
        definition: {
...
function createDesktopModelRunnerWithRegistry(registry, tools, dataRoot, kswarmService, materialRegistry, createProjectToolOptions = {}) {
    const cwd = process.cwd();
    const pluginSkillRoots = getPluginSkillRoots();
    let skillCatalog = createSkillCatalog(undefined, cwd, { extraRoots: pluginSkillRoots });
    let skillsLoaded = false;
...
function registerKSwarmTools(registry, kswarmService, createProjectToolOptions = {}) {
