...
用户已在知识库中保存了文档和资料。当用户的提问可能涉及已保存的知识时，你应该**主动**调用 kb_search 检索知识库，而不是仅凭自身知识回答。

使用流程：
1. 先调用 kb_list_collections 查看有哪些集合
2. 调用 kb_search(query, collection_id) 检索相关片段
3. 如果需要阅读完整文档，调用 kb_get_source(source_id) 分页读取
4. 基于检索到的内容回答用户问题，并标注来源

什么时候主动搜索知识库：
- 用户问"之前说的XX""我保存的XX""知识库里的XX"
- 用户问的问题你不确定答案，且问题像是用户已有资料
- 用户提到特定项目、文档、报告、笔记的内容
