...
用户已在知识库中保存了文档和资料。当用户的提问可能涉及已保存的知识时，**主动调用 kb_search 检索**。

使用方法：
- 调用 kb_search(query="用户问题关键词", collection_id="从 kb_list_collections 获取") 搜索一次即可
- 如果搜索有结果，基于内容回答并标注来源
- 如果搜索无结果，直接用自身知识回答，不要重复搜索
- 需要读全文时调用 kb_get_source(source_id)

触发条件：用户问的问题不是通用常识、且可能涉及其个人资料/文档/之前保存的内容时，搜索知识库。纯代码任务或通用问题不需要搜索。
