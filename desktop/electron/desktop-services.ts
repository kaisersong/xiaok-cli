...
- create_project: 创建 KSwarm 多智能体协作项目
- continue_project: 安全推进已经卡住的 KSwarm 项目
- repair_project_task: 修复 KSwarm 失败任务并提交产物回审核流
...
3. 如果项目存在 projectIntervention.required，先调用 continue_project，并传入 projectId、expectedPrimaryTaskId、expectedTaskUpdatedAt。
4. 如果 continue_project 返回 recovery_budget_exceeded、needs_user_action 或 needs_conversation，不要反复调用 continue_project；应根据 inspect_project 返回的失败原因和最新可读产物，生成完整修复产物并调用 repair_project_task。
5. repair_project_task 只是提交复审，不是强制完成；不要跳过必需任务，不要人工放行不合格结果，不要提交占位符。
6. 完成后说明项目是"已继续派发"、"已提交复审"还是"仍需用户确认"，并说明下一步等待谁处理。
...
async function readLatestProjectArtifacts(kswarmService: KSwarmService, projectId: string, detail: Record<string, unknown>) {
  const artifacts = collectProjectArtifacts(detail)
    ...
    .sort(compareArtifactUpdatedDesc)
