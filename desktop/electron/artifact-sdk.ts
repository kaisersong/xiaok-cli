...
export const ARTIFACT_SDK_CODE = `(function() {
  ...
  function ensureShadow() {
    ...
    style.textContent = ':host{all:initial;position:fixed;z-index:2147483647;left:0;top:0;pointer-events:none}.xiaok-text-highlight{position:fixed;pointer-events:none;background:rgba(59,130,246,.2);border-radius:2px;box-shadow:0 0 0 1px rgba(59,130,246,.4)}.xiaok-annotation-card{position:fixed;pointer-events:all;width:min(320px,calc(100vw - 24px));padding:12px;border-radius:12px;background:#1e293b;color:#f1f5f9;border:1px solid #3b82f6;box-shadow:0 20px 60px rgba(0,0,0,.4);font:14px/1.4 -apple-system,BlinkMacSystemFont,sans-serif}.xiaok-annotation-card textarea{width:100%;min-height:80px;resize:vertical;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#f1f5f9;padding:8px;font:inherit}.xiaok-annotation-card .xiaok-row{display:flex;gap:8px;justify-content:flex-end;margin-top:8px}.xiaok-annotation-card button{border:0;border-radius:8px;padding:8px 12px;font-size:13px;font-weight:600;cursor:pointer}.xiaok-annotation-card .xiaok-send{background:#3b82f6;color:#fff}.xiaok-annotation-card .xiaok-send:hover{background:#2563eb}.xiaok-annotation-card .xiaok-cancel{background:#334155;color:#f1f5f9}';
  ...
  function showAnnotationCard(target, options) {
    ...
    var heading = c.tag === 'text' ? '\u6807\u6ce8\u6587\u5b57' : '\u6807\u6ce8 <' + c.tag + '>';
    var placeholder = c.tag === 'text'
      ? '\u544a\u8bc9 Agent \u4f60\u60f3\u600e\u4e48\u4fee\u6539\u8fd9\u6bb5\u6587\u5b57...'
      : '\u544a\u8bc9 Agent \u4f60\u60f3\u600e\u4e48\u4fee\u6539\u8fd9\u4e2a\u5143\u7d20...';
    card.innerHTML = '<div style="font-weight:700;margin-bottom:6px">' + heading + '</div><textarea placeholder="' + placeholder + '"></textarea><div class="xiaok-row"><button class="xiaok-cancel" type="button">\u53d6\u6d88</button><button class="xiaok-send" type="button">\u53d1\u9001\u6807\u6ce8</button></div>';
