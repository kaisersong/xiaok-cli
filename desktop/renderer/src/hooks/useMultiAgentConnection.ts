...
export function useMultiAgentConnection(threadId?: string, groupId?: string, enabled = true) {
  const api = getDesktopApi();
  const facts = useMemo(() => new MultiAgentThreadFacts(threadId ?? ''), [api, threadId]);
  const connection = useMemo(() => enabled && threadId && typeof api?.subscribeMultiAgents === 'function'
    ? new MultiAgentConnection(api, threadId, groupId, facts) : null, [api, threadId, groupId, facts, enabled]);
