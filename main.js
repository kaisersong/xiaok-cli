...
import { buildBrowserWindowOptions, isAllowedNavigationUrl, isAllowedShellExternalUrl, resolveLocalFileOpenPath, } from './security.js';
import { findIntentBrokerProtocolUrl, registerIntentBrokerProtocolClient, } from './intent-broker-protocol.js';
import { resolveDesktopDockIconPath, resolveDesktopWindowIconPath } from './window-icon.js';
import { attachCloseToMinimize, attachWindowRepaintHandlers, removeWindowsWindowMenu, restoreExistingWindow, } from './window-lifecycle.js';
...
import { createKSwarmRuntimeBridge, createKSwarmRuntimeBridgeBrokerClient, submitKSwarmRuntimeResultToBroker, submitKSwarmWorkflowNodeResultToBroker, } from './kswarm-runtime-bridge.js';
import { XIAOK_PO_SEED_ID, XIAOK_WORKER_SEED_ID } from '../shared/kswarm-seed-contract.js';
const __dirname = dirname(fileURLToPath(import.meta.url));
let mainWindow = null;
let isQuitting = false;
function debugMain(message, extra) {
...
async function createWindow() {
    ...
    ipcMain.handle('desktop:kswarm:resumeWorkflowRun', (_event, input) => resumeOneScriptWorkflow(kswarmService, input?.projectId, input?.workflowRunId));
    let restartRuntimeBridgeService = async () => { };
    ...
    ipcMain.handle('desktop:services:restart', (_event, serviceId) => (serviceId === 'runtime-bridge'
    ...
    ipcMain.handle('desktop:getUpdateStatus', () => {
        try {
            ...
            return { checking: false, available: false, downloading: false, downloaded: false, progress: 0, error: e.message, currentVersion: app.getVersion() };
        }
    });
    ...
    let mcpDispose;
    const runtimeBridgeClients = [];
    let runtimeBridgeStarted = false;
    let runtimeBridgeFallbackTimer = null;
    const stopRuntimeBridge = () => {
        ...
        for (const client of runtimeBridgeClients.splice(0))
            client.stop();
    ...
    const startRuntimeBridge = () => {
        ...
        void kswarmStartPromise.then(() => {
            ...
            const runtimeBridge = {
                ...
                handleAssignPo: (input) => services.runKSwarmAssignPo(input),
                handleReviewSubmission: (input) => services.runKSwarmReviewSubmission(input),
                handlePlanApproved: (input) => services.runKSwarmPlanApproved(input),
                handleReadinessProbe: (input) => services.runKSwarmReadinessProbe(input),
            };
            runtimeBridgeClients.push(createKSwarmRuntimeBridgeBrokerClient({
                brokerUrl,
            ...
            for (const client of runtimeBridgeClients) {
                client.start().catch((error) => {
                    console.warn('[main] Failed to start kswarm runtime bridge client:', error.message);
                });
            }
        });
    };
    ...
    const timedActionScheduler = new TimedActionScheduler(timedActionStore, {
        ...
        onRunComplete: (event) => {
            if (event.action.executor.kind !== 'agent_task')
                return;
    ...
    ipcMain.handle('desktop:cancelScheduledTask', (_event, id) => {
    ...
    ipcMain.handle('desktop:getTimedActionRuns', (_event, actionId) => {
        return timedActionService.getRuns(actionId);
    });
    ipcMain.handle('desktop:timedAction:approveAuto', (_event, actionId) => {
        return timedActionService.approveAuto(actionId) ?? null;
    });
    ipcMain.handle('desktop:timedAction:revokeAuto', (_event, actionId) => {
    ...
    ipcMain.handle('desktop:createReminder', (_event, input) => {
    ...
    ipcMain.handle('desktop:cancelReminder', (_event, id) => timedActionService.cancelReminder(id));
    ...
    ipcMain.handle('desktop:saveSkillDebugConfig', (_event, input) => services.saveSkillDebugConfig(input));
