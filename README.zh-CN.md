...
| 指标 | xiaok v0.7.3 | Claude Code | 提升 |
|------|-------------|-------------|------|
| **自主性得分** | 100% | 100% | — |
| **简单问答延迟** | 3.8s | 7.5s | **-49%** |
...
- **并行任务执行**：多个 Thread 的任务并发运行，互不干扰
- **桌面版 v0.5.5**：MCP 插件集成、Skill 自动匹配、多轮上下文、并行任务
- **报告渲染器修复**：修复生成报告中的 HTML 转义和日期格式问题
- **Windows 支持**：通过 GitHub Actions 构建 Windows 安装包
