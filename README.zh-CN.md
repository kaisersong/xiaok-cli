...
v1.3.8 在 KSwarm 项目之上加入了第一版基础动态工作流能力。它还不是面向用户的通用 workflow builder，也还不是 PO agent 动态生成任意脚本，但已经是一条真实的 durable workflow runtime 切片：
...
这条实现明确了 Xiaok 的 dynamic workflow 产品方向：KSwarm 继续作为项目控制层，workflow orchestration 在 agent 层执行；当前从内置 workflow 起步，后续可以演进到更丰富的动态生成执行计划。
...
- **xiaok-1.3.8-arm64.dmg** — macOS DMG 安装包（Apple Silicon）
- **xiaok-1.3.8-arm64-mac.zip** — macOS ZIP 包（Apple Silicon）
- **xiaok-setup-1.3.8.exe** — Windows 安装包（x64）
...
- **基础版 Dynamic Workflow**：以持久化 workflow run 运行项目快速诊断和 Agent 复核诊断，包含 Worker、Reviewer 和 gate 阶段
