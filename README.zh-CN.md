...
- **空间执行**：已授权工作区默认允许本机命令执行，保留手动关闭的选择；支持空间定时任务、已配置连接器和工作区材料读取。本机命令使用当前电脑账户权限，工作目录不是操作系统沙箱；受管理的命令执行目前支持 macOS/POSIX，Windows 暂不可用。
- **协作体验**：智能体显示可读名称，支持空间别名；工作说明发布反馈更清楚；项目智能体可选择模型并回退到当前模型。
- **桌面导航**：恢复新建任务、自动化、项目、协作空间、知识库五个主菜单，下方任务／定时／项目／协作 Tab 独立浏览实例。
- **消息阅读**：定时触发、对应智能体回复及工作区通知默认折叠；进入空间定位最新消息，上翻后可点击箭头回到底部。
- **执行恢复**：中断时保留已完成工具结果并修复不完整调用历史；交互问题不再使用普通工具超时限制。
- CLI 与 Desktop 版本统一为 `1.5.5`，内置关联仓库固定到 `desktop-v1.5.5`；更新版本元数据不等于发布 npm。
