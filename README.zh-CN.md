...
- **聚焦测试和 E2E 覆盖**：本版本覆盖 eager parallel call 拒绝、runtime 分支标注、KSwarm 并行分组持久化、HTTP contract、后台工具启动，以及一条通过 KSwarm 和 desktop runtime bridge 完成的动态脚本 workflow E2E。

这是 dynamic workflow orchestration 的基础版本，不是完整开放式用户 workflow 平台。跨应用重启的 script job recovery、高级 failure policy、专业 workflow 模板和质量 eval 仍属于后续阶段。
