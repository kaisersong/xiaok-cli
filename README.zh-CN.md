...
| 指标 | xiaok v0.7.4 | Claude Code | 提升 |
|------|-------------|-------------|------|
| **自主性得分** | 100% | 100% | — |
| **简单问答延迟** | 3.8s | 7.5s | **-49%** |
...
- **鼠标跟踪修复**：防止终端鼠标跟踪序列污染输入栏（Ghostty、iTerm2）
- **未知 CSI 序列处理**：未识别的 ANSI 转义序列被完整消费，不再泄漏为文本
- **工具结果溢出**：大型工具结果溢出到磁盘，不再静默截断
- **桌面版提醒优化**：改进每日/每周提醒创建，移除无用的 intent delegation 开销
...
- **xiaok-0.5.5-arm64.dmg** — macOS DMG 安装包（Apple Silicon）
- **xiaok-0.5.5-arm64-mac.zip** — macOS ZIP 包（Apple Silicon）
- **xiaok-0.5.5-setup.exe** — Windows 安装包（x64）
