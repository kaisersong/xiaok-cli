...
- **发布打包刷新**：macOS 与 Windows 桌面产物统一从 1.3.0 源码和插件 bundle 基线构建
...
- **xiaok-1.3.0-arm64.dmg** — macOS DMG 安装包（Apple Silicon）
- **xiaok-1.3.0-arm64-mac.zip** — macOS ZIP 包（Apple Silicon）
- **xiaok-1.3.0-setup.exe** — Windows 安装包（x64）
...
**v1.3.0** — Desktop + KSwarm 可靠性版本：为 CLI agent 增加 runtime 探测和健康冷却，加入卡住运行 watchdog telemetry，失败重试重新走能力路由；PPTX/HTML/Markdown 任务进入 PO 验收前做强交付物校验，提供确定性的本地 PPTX 兜底执行器，并修复 PO 制定计划中断后项目无法继续的问题。
