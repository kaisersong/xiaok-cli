...
v1.3.9 在 KSwarm 项目之上扩展了基础动态工作流能力。它还不是面向用户的通用 workflow builder，也还不是执行 raw JavaScript 的开放平台，但已经是一条真实的 durable workflow runtime 切片：
...
- **任务级工作流执行**：任务卡片可以针对当前任务打开 `po-generated-task-workflow` proposal。proposal 会带 task scope、预算、权限和验收标准，必须人工确认后才 dispatch。
- **受控 PO 生成建议**：KSwarm 可以根据项目/任务上下文生成 validated workflow IR。第一版是受控模板，用来验证 proposal 和审批链路；不会执行模型生成的 raw JavaScript 或任意用户脚本。
...
- **xiaok-1.3.9-arm64.dmg** — macOS DMG 安装包（Apple Silicon）
- **xiaok-1.3.9-arm64-mac.zip** — macOS ZIP 包（Apple Silicon）
- **xiaok-setup-1.3.9.exe** — Windows 安装包（x64）
...
- **基础版 Dynamic Workflow**：以持久化 workflow run 运行项目快速诊断、Agent 复核诊断和任务级 PO-generated workflow proposal，包含预算、缓存、恢复、进度、Reviewer 和 gate 元数据
