...
export async function createStatelessOpenAIResponse(params) {
...
export async function compactOpenAIResponsesContext(params) {
    const validated = validateRequestParams(params);
...
function validatePublicOpenAIBaseUrl(baseUrl) {
...
async function postOpenAIResponse(params) {
...
function validateResponsePayload(payload, expectedObject) {
    ...
    if (expectedObject === 'response.compaction') {
        const hasCanonicalCompaction = output.some((item) => (item.type === 'compaction'
...
function validateUsage(value) {
    ...
    if (value.input_tokens_details !== undefined) {
        ...
        if (value.input_tokens_details.cached_tokens !== undefined) {
            cachedInputTokens = requireNonNegativeInteger(value.input_tokens_details.cached_tokens, 'usage.input_tokens_details.cached_tokens');
        }
    }
    return {
        inputTokens,
...
function validateToolHistory(input) {
...
function requireExactNonEmptyString(value, field) {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new Error(`OpenAI native ${field} must be a non-empty string`);
    }
    return value;
}
function validateFunctionCallOutput(value, callId) {
    if (typeof value === 'string')
        return;
    if (!Array.isArray(value) || value.length === 0) {
        throw new Error(`function_call_output output must be a string or non-empty content array for call_id: ${callId}`);
    }
    for (const content of value) {
        ...
        if (content.type === 'input_image') {
            if (!hasNonEmptyString(content.image_url)
        ...
        if (content.type === 'input_file') {
            if (!hasNonEmptyString(content.file_data)
                && !hasNonEmptyString(content.file_id)
