...
import { discoverSkills, parseFrontmatter, } from './loader.js';
...
function inferAliases(filePath) {
...
function validateParsedFrontmatter(parsed, filePath) {
    const issues = [];
...
function findConflicts(filePath, parsed, catalog) {
    ...
    for (const skill of catalog) {
        ...
        for (const token of currentTokens) {
            ...
            return [
                issue('error', 'name_conflict', `Skill command "${token}" already exists at ${skill.path}. Choose a different name or alias.`),
            ];
        }
    }
    return [];
}
export async function validateSkillFile(filePath, options = {}) {
    ...
    const catalog = await discoverSkills(options.xiaokConfigDir ?? getConfigDir(), options.cwd ?? process.cwd(), options);
