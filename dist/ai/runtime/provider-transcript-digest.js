...
function canonicalizeBlock(block, role) {
    if (block.type === 'text') {
...
function canonicalizeMessages(messages) {
    const canonicalMessages = new CapturedArray(messages.length);
    for (let messageIndex = 0; messageIndex < messages.length; messageIndex += 1) {
        const message = messages[messageIndex];
        const content = new CapturedArray(message.content.length);
        for (let blockIndex = 0; blockIndex < message.content.length; blockIndex += 1) {
            content[blockIndex] = canonicalizeBlock(message.content[blockIndex], message.role);
        }
