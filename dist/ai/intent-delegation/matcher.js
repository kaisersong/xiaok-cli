...
function scoreField(queryTokens, values, weight) {
    let score = 0;
    let matched = false;
...
export function matchSkillsForTask(query, skills, limit = 5) {
    const queryTokens = tokenize(query);
    const matches = skills.map((skill) => {
        let score = 0;
        const reasons = [];
        const descriptionResult = scoreField(queryTokens, [skill.description, skill.whenToUse ?? ''], FIELD_WEIGHTS.description);
        ...
        if (taskGoalsResult.matched)
            reasons.push('task-goals');
        ...
        if (inputKindsResult.matched)
            reasons.push('input-kinds');
