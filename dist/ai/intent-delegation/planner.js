...
import { extractProvidedSourcePaths, stripProvidedSourcePaths, } from './path-contract.js';
...
export function createIntentPlan(input) {
    ...
    const allowActiveIntentFallback = Boolean(input.activeIntent && (isSupplementOrClarification(rawIntent) || hasContinuationCue(rawIntent)));
    const allowActiveShortReply = Boolean(input.activeIntent && (isSupplementOrClarification(rawIntent) || hasContinuationCue(rawIntent)));
...
function isAcknowledgement(input) {
...
function isLikelyInformationalQuery(input) {
...
function isNonSubstantial(input, activeIntent) {
    if (input.length <= 2) {
        return true;
    }
...
function isSupplementOrClarification(input) {
    return SUPPLEMENT_PATTERNS.some((pattern) => pattern.test(input));
}
function looksLikeDelegatedWorkRequest(rawIntent, providedSourcePaths, extracted, activeIntent, workRequestCue) {
...
function detectContinuationMode(rawIntent, finalDeliverable, activeIntent) {
    if (!activeIntent) {
        return 'new_intent';
    }
...
function resolveTopLevelIntentType(rawIntent, stageSpecs, activeIntent, continuationMode) {
...
function classifyByRules(rawIntent) {
    if (containsAny(rawIntent, ANALYZE_HINTS))
        return 'analyze';
    if (containsAny(rawIntent, SUMMARIZE_HINTS))
        return 'summarize';
    if (containsAny(rawIntent, REVISE_HINTS))
        return 'revise';
    if (containsAny(rawIntent, GENERATE_HINTS))
        return 'generate';
    return 'generate';
}
function containsAny(input, hints) {
    return hints.some((hint) => input.includes(hint));
}
function extractIntentShape(rawIntent, providedSourcePaths, activeIntent, allowActiveIntentFallback = false) {
...
function extractConstraints(rawIntent) {
    const patterns = [
        /(控制在[^，。！？,!.?]+)/u,
        /(限制在[^，。！？,!.?]+)/u,
        /(不要[^，。！？,!.?]+)/u,
...
function extractBoundary(rawIntent) {
...
function extractSingleDeliverable(rawIntent) {
    ...
    if (rawIntent.includes('报价测算'))
        return '报价测算';
    if (rawIntent.includes('关键结论'))
        return '关键结论';
    if (rawIntent.includes('产品方案'))
        return '产品方案';
    return '';
}
function extractSequencedStageSpecs(rawIntent) {
    ...
    const matches = Array.from(rawIntent.matchAll(new RegExp(`(?:先|然后|再|并且|并|接着|之后)?\\s*(${stageVerbPattern})(?:一版|一个|一份|一篇)?([^，。！？,!.?；;]+?)(?=(?:然后|再|并且|并|接着|之后)\\s*(?:${stageVerbPattern})|[，。！？,!.?；;]|$)`, 'gu')));
    const specs = matches
        .flatMap((match) => {
...
function cleanupDeliverable(value) {
    return value
...
function buildStageLabel(intentType, deliverable) {
...
function deriveSegmentationConfidence(rawIntent, stageSpecs) {
...
function inferDeliverableFamily(value) {
    if (!value)
        return 'unknown';
    ...
    if (/(表格|清单|表|csv|sheet)/iu.test(value))
        return 'table';
    if (/(方案|报告|提纲|文案|稿|说明|proposal|report|brief)/iu.test(value))
        return 'document';
    return 'unknown';
}
function deriveRiskTier(intentType, continuationMode, intentMode) {
    if (continuationMode === 'clarify')
        return 'medium';
    if (intentType === 'analyze')
        return 'medium';
    if (intentMode === 'multi_stage')
        return 'medium';
    return 'low';
}
function buildStages(input) {
    return input.stageSpecs.map((spec, index) => {
        ...
        const steps = template.steps.map((step, stepIndex) => {
            ...
            return {
                ...
                order: stepIndex,
                role: step.role,
                skillName: chooseSkillName(step.role, step.fallbackRoles ?? [], spec.intentType, spec.deliverable, spec.rawSegment || input.rawIntent, input.skills, input.skillScoreLookup),
...
function chooseSkillName(role, fallbackRoles, intentType, deliverable, rawIntent, skills, skillScoreLookup) {
...
function pickBestSkill(role, intentType, deliverable, rawIntent, skills, skillScoreLookup) {
...
function scoreRoleMatch(skill, role, intentType, deliverable) {
    ...
    const ioTokens = new Set(tokenize(`${skill.taskHints.inputKinds.join(' ')} ${skill.taskHints.outputKinds.join(' ')}`));
    ...
    const hasStructuredHints = (skill.taskHints.taskGoals.length
        + skill.taskHints.inputKinds.length
        + skill.taskHints.outputKinds.length) > 0;
    const hasDeliverableEvidence = intersects(deliverableTokens, ioTokens) || intersects(deliverableTokens, descriptionTokens);
    let score = 0;
    if (intersects(roleTokens, taskGoalTokens))
        score += 5;
    if (intersects(new Set([intentType]), taskGoalTokens) || intersects(new Set([intentType]), descriptionTokens)) {
        score += 3;
    }
    if (intersects(deliverableTokens, ioTokens))
        score += 4;
    if (intersects(deliverableTokens, descriptionTokens))
        score += 4;
    if (intersects(intentTokens, descriptionTokens) || intersects(intentTokens, taskGoalTokens))
        score += 2;
    if (intersects(roleTokens, ioTokens))
        score += 2;
    if (intersects(roleTokens, descriptionTokens))
        score += 1;
...
function buildStableId(prefix, seed, suffix) {
