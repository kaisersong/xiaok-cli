...
export async function renderPromptSections(opts) {
    ...
    sections.push('当用户要求安装 skill、扩展技能，或提到一个本地不存在的 skill 时，不要只检查本地 catalog 后放弃。先用 web_search / web_fetch 到 GitHub、ClawHub 等来源定位可靠的 skill Markdown 文件，再调用 install_skill 安装到 project 或 global scope。');
    sections.push('当用户要求删除、卸载某个已安装 skill 时，优先调用 uninstall_skill；卸载后如果还需要继续使用，应重新检查当前 catalog，而不是假设旧 skill 仍然存在。');
