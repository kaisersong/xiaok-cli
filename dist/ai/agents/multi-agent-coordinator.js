...
const FINAL_STATUSES = new Set(['completed', 'failed', 'interrupted', 'closed']);
...
export class MultiAgentCoordinator {
    ...
    executionMode;
    externalMailbox;
    preparedTurns = new Map();
    constructor(options = {}) {
        this.executionMode = options.executionMode ?? 'automatic';
        this.externalMailbox = options.externalMailbox;
        this.maxResidentAgents = clampInteger(options.maxResidentAgents, DEFAULT_MAX_RESIDENT_AGENTS, 2, 16);
        this.maxDepth = clampInteger(options.maxDepth, DEFAULT_MAX_DEPTH, 1, 16);
        this.maxMessageChars = clampInteger(options.maxMessageChars, DEFAULT_MAX_MESSAGE_CHARS, 1, 100_000);
        this.maxInboxMessages = clampInteger(options.maxInboxMessages, DEFAULT_MAX_INBOX_MESSAGES, 1, 1_000);
        this.maxWaitMessages = clampInteger(options.maxWaitMessages, DEFAULT_MAX_WAIT_MESSAGES, 1, 100);
        this.maxResultChars = clampInteger(options.maxResultChars, DEFAULT_MAX_RESULT_CHARS, 100, 100_000);
        this.closeSettlementTimeoutMs = clampInteger(options.closeSettlementTimeoutMs, DEFAULT_CLOSE_SETTLEMENT_TIMEOUT_MS, 1, 2_000);
    ...
    async spawn(input) {
        if (this.executionMode === 'externally_activated')
            throw new Error('external mode requires prepareSpawn and applied activation');
    ...
    prepareFollowup(input) {
        ...
        this.assertMessage(input.message);
        if (this.isInClosingSubtree(record) || record.status === 'closed' || record.resourcesReleased)
            throw new Error('agent is closed');
        if (record.execution || record.controller || record.preparedTurnId)
            throw new Error('agent is busy; followup requires later external admission');
        if (record.cleanupError)
            throw new Error(`agent cleanup failed: ${record.cleanupError}`);
        if (!record.runtimeResident)
            this.assertRuntimeCapacity();
    ...
    activatePreparedTurn(handle, options = {}) {
        this.assertExternalMode();
        const prepared = this.requirePreparedTurn(handle);
        const record = prepared.record;
        if (this.disposed || prepared.state !== 'prepared' || this.isInClosingSubtree(record) || record.status !== 'pending')
            throw new Error('prepared turn is not activatable');
    ...
    rollbackPrepared(handle) {
        this.assertExternalMode();
        const prepared = this.requirePreparedTurn(handle);
        if (prepared.state === 'rolled_back')
            return;
        const record = prepared.record;
        if (prepared.state === 'activated' || record.execution || record.controller)
            throw new Error('activated/live turn cannot be rolled back');
        ...
        if (prepared.kind === 'spawn') {
            ...
            if (this.canonicalIds.get(record.canonicalName) === record.id)
                this.canonicalIds.delete(record.canonicalName);
        }
        else
            Object.assign(record, prepared.previous);
        this.notifyWaiters();
    }
    reservePreparedTurn(record, message, kind, previous) {
        const handle = { reservationId: randomUUID(), agentId: record.id, expectedTurn: record.turn, snapshot: this.snapshot(record) };
    ...
    requirePreparedTurn(handle) {
        const prepared = this.preparedTurns.get(handle.reservationId);
        if (!prepared || prepared.handle.agentId !== handle.agentId || prepared.handle.expectedTurn !== handle.expectedTurn)
            throw new Error('unknown or stale prepared turn');
        return prepared;
    }
    assertExternalMode() {
        if (this.executionMode !== 'externally_activated')
            throw new Error('prepare API requires externally_activated mode');
    }
    sendMessage(input) {
        if (this.executionMode === 'externally_activated') {
            if (!this.externalMailbox)
                throw new Error('external mailbox is service-owned');
    ...
    followupTask(input) {
        if (this.executionMode === 'externally_activated')
            throw new Error('external mode requires prepareFollowup and applied activation');
        this.assertMutationSource(input.requestSource);
        const caller = this.requireCaller(input.callerId);
        const target = this.resolveTarget(input.target);
        this.assertMessage(input.message);
    ...
    schedule(record, externalController) {
        if (record.execution || this.isClosed(record) || record.cleanupError)
            return;
        ...
        record.execution = execution.finally(async () => {
            if (!record.closing && record.queue.length === 0 && !record.cleanupError) {
                try {
                    if (record.session?.suspend) {
                        ...
                        if (!record.closing && record.queue.length === 0)
                            record.runtimeResident = false;
                    }
            ...
            if (externalController && record.controller === externalController)
    ...
    async processQueue(record, externalController) {
        while (record.queue.length > 0 && !this.isClosed(record) && !record.cleanupError) {
            ...
            try {
                ...
                if (!record.session) {
                    ...
                    if (!createSession || record.parentId === null)
                        throw new Error('agent session factory is unavailable');
                    const parent = this.records.get(record.parentId);
                    if (!parent)
                        throw new Error(`parent agent not found: ${record.parentId}`);
                ...
                if (this.isClosed(record) || record.timedOut)
                    return;
                if (controller.signal.aborted)
                    throw abortError();
    ...
    withInboxContext(record, message) {
        const pending = this.takeInboxInput(record);
        return pending ? `${pending}\n\n${message}` : message;
    }
    takeInboxInput(record) {
        if (record.inbox.length === 0)
            return undefined;
        const pending = record.inbox.splice(0, this.maxWaitMessages);
        for (const item of pending)
            this.publish(record, 'message_consumed', item);
        const lines = pending.map((item) => `[${this.displayName(item.senderId)}] ${item.text}`);
        return `<inter_agent_messages>\n${lines.join('\n')}\n</inter_agent_messages>`;
    }
    notifyParent(record, text, kind) {
        if (this.executionMode === 'externally_activated')
            return;
        if (!record.parentId)
            return;
        const parent = this.records.get(record.parentId);
        if (!parent || parent.status === 'closed' || parent.closing || parent.inbox.length >= this.maxInboxMessages)
            return;
    ...
    async closeRecord(record, preserveFailure = false) {
            ...
            const cleanup = (async () => {
                ...
                if (session) {
                    if (session !== existingSession)
                        await cleanupStep(() => session.deactivate?.());
    ...
    assertMutationSource(source) {
        ...
        if (this.disposed)
            throw new Error('multi-agent coordinator is disposed');
    }
    assertRuntimeCapacity() {
        ...
        if (residents.length < this.maxResidentAgents)
            return;
    ...
    requireCaller(callerId) {
        ...
        if (caller.status === 'closed')
            throw new Error(`caller agent is closed: ${caller.canonicalName}`);
    ...
    assertDescendantMutationAllowed(source, caller, target, operation) {
    ...
    isDescendantOf(target, ancestorId) {
    ...
    drainMessages(caller, targetIds) {
        const matched = [];
        const remaining = [];
        for (const message of caller.inbox) {
            if (targetIds.has(message.senderId) && matched.length < this.maxWaitMessages)
        ...
        for (const message of matched)
            this.publish(caller, 'message_consumed', message);
        return matched;
    }
    createMessage(senderId, receiverId, text, kind) {
        return {
    ...
    startWatchdog(record, controller) {
        let idleTimer;
        let totalTimer;
        ...
        const timeout = (code) => {
            ...
            for (const child of subtree)
                void this.closeRecord(child, child.id === record.id);
            this.notifyWaiters();
        };
        const armIdle = () => {
    ...
    waitForSignal(timeoutMs, signal) {
