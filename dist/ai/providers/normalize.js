...
import { isOfficialKimiK3OpenAIEndpoint, resolveModelRuntimeOptions, } from './model-runtime-options.js';
...
function detectKnownProvider(baseUrl) {
    const normalizedBaseUrl = (baseUrl ?? '').toLowerCase();
    try {
    ...
    if (normalizedBaseUrl.startsWith('https://api.deepseek.com'))
        return 'deepseek';
    if (normalizedBaseUrl.startsWith('https://open.bigmodel.cn'))
        return 'glm';
    if (normalizedBaseUrl.startsWith('https://api.minimax.chat'))
        return 'minimax';
    if (normalizedBaseUrl.startsWith('https://generativelanguage.googleapis.com'))
        return 'gemini';
    return null;
}
function buildFirstPartyConfig(providerId, overrides, modelOverride) {
...
function normalizeLegacyConfig(config) {
    ...
    const protocol = isClaudeCompatibleCustomEndpoint(customBaseUrl, config.models.custom?.model)
        ? 'anthropic'
        : 'openai_legacy';
    return {
        schemaVersion: 2,
...
function withNormalizedDefaults(config) {
    return {
        ...config,
        intentBoundary: {
