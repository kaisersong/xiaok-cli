...
export function createIntentDelegationTools(options) {
    return [
        {
            permission: 'safe',
            definition: {
...
function buildIntentPlanDraftFromPlanner(plan, instanceId, intentId) {
    return {
        ...plan,
        instanceId,
...
async function loadLedgerOrThrow(ledgerStore, sessionId) {
...
function resolveScopedInstanceId(value, expected) {
    if (expected?.trim()) {
...
function requireIntentType(value) {
    const intentType = requireNonEmptyString(value, 'intent_type');
...
function requireRiskTier(value) {
    const riskTier = requireNonEmptyString(value, 'risk_tier');
...
function requireArtifactStorage(value) {
    const storage = requireNonEmptyString(value, 'storage');
    if (!['inline', 'file_ref'].includes(storage)) {
        throw new Error(`非法 storage: ${storage}`);
    }
    return storage;
}
function requireValidationState(value, field) {
...
function requireStepStatus(value) {
    const stepStatus = requireNonEmptyString(value, 'step_status');
    if (!['running', 'blocked', 'completed', 'failed'].includes(stepStatus)) {
        throw new Error(`非法 step_status: ${stepStatus}`);
    }
    return stepStatus;
}
function resolveTurnIntentPlan(plan, sessionId) {
...
function requireContinuationMode(value) {
    if (value === undefined) {
        return undefined;
    }
    const continuationMode = requireNonEmptyString(value, 'continuation_mode');
