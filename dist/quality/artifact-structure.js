...
export function resolveStructuralKind(filePath) {
...
export function validateArtifactStructure(filePath, kind) {
    try {
        if (kind === 'pdf')
            return validatePdfStructure(filePath);
        if (kind === 'pptx')
            return validatePptxStructure(filePath);
        return { ok: true };
    }
    catch {
        return { ok: true };
...
function validatePptxStructure(filePath) {
    const fd = openSync(filePath, 'r');
    try {
...
export function validateArtifactBytes(kind, buf, bytesRead) {
    if (kind === 'pdf') {
