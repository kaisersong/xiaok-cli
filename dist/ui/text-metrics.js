...
export function clampOffset(text, offset) {
...
export function moveOffsetLeft(text, offset) {
    return clampOffset(text, offset - 1);
}
export function moveOffsetRight(text, offset) {
    return clampOffset(text, offset + 1);
}
export function offsetToDisplayColumn(text, offset) {
    const symbols = splitSymbols(text);
    return getDisplayWidth(symbols.slice(0, clampOffset(text, offset)).join(''));
}
export function sliceByDisplayColumns(text, start, width) {
    if (width <= 0)
        return '';
    let column = 0;
    let result = '';
