...
export class ScrollRegionManager {
    ...
    positionCursorForInput() {
        ...
        const cursorVisibleLine = Math.max(0, Math.min(state.cursorLine - state.visibleStart, state.visibleLines.length - 1));
    ...
    padLine(line, width, hasBackground = false) {
        ...
        if (visibleLen >= width)
            return line.slice(0, width + (line.length - visibleLen));
