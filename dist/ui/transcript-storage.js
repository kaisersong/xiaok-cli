import { chmodSync, closeSync, createReadStream, createWriteStream, existsSync, fstatSync, fsyncSync, mkdirSync, openSync, readSync, readFileSync, readdirSync, renameSync, statSync, unlinkSync, writeSync, } from 'node:fs';
...
import { PluginClaimLock, PluginLockBusyError, PluginLockFailClosedError, } from '../platform/provider-store/plugin-claim-lock.js';
import { probeIdentityOrFailClosed, probeProcessIdentity, } from '../platform/provider-store/process-identity.js';
...
export function transcriptPaths(sessionId, rootDir = defaultTranscriptRoot()) {
...
export function validateTranscriptSessionId(sessionId) {
    if (!SESSION_ID.test(sessionId) || sessionId === '.' || sessionId === '..' || sessionId.length > 200) {
        throw new TranscriptStorageError('invalid_transcript_session_id', `invalid transcript session id: ${sessionId}`);
    }
}
export async function acquireTranscriptLease(sessionId, rootDir = defaultTranscriptRoot(), acquireTimeoutMs = 10_000) {
    ...
    if (selfProbe.kind !== 'alive') {
        throw new TranscriptStorageError('transcript_lock_identity_unreadable', selfProbe.kind === 'unknown' ? selfProbe.diagnostic : 'current process identity not found');
    }
    ...
    try {
        ...
        if (error instanceof PluginLockFailClosedError || isIdentityProbeError(error)) {
            throw new TranscriptStorageError('transcript_lock_identity_unreadable', `cannot verify session claim owner for ${sessionId}`, { cause: error });
        }
        throw error;
    }
}
export async function prepareTranscriptWriter(sessionId, rootDir = defaultTranscriptRoot()) {
    const paths = transcriptPaths(sessionId, rootDir);
    ...
    if (existsSync(paths.raw))
        chmodOwnerWritable(paths.raw);
}
export function sealTranscriptWriter(sessionId, rootDir = defaultTranscriptRoot(), operations = {
    open: openSync,
    fsync: fsyncSync,
    close: closeSync,
}) {
...
export function readTranscriptJsonValues(sessionId, rootDir = defaultTranscriptRoot()) {
        ...
        for (const source of sources) {
            ...
            for (const entry of parseBufferLines(bytes)) {
                ...
                try {
                    ...
                    throw new TranscriptStorageError('transcript_invalid_json', `invalid transcript JSON at line ${entry.lineNumber}`, { cause: error });
                }
            }
        }
    }
...
export async function* iterateTranscriptLines(sessionId, rootDir = defaultTranscriptRoot(), options = {}) {
    ...
    try {
        for (const source of sources) {
            ...
            const stream = source.gzip ? fileStream.pipe(createGunzip()) : fileStream;
...
export async function archiveTranscript(sessionId, options = {}) {
    ...
    try {
        mkdirSync(paths.rootDir, { recursive: true });
        const recovered = await recoverArchivePending(paths, sessionId, olderThanDays, options.now ?? Date.now(), options.onPhase);
        if (recovered)
            return recovered;
        if (!existsSync(paths.raw)) {
            const manifest = loadLatestManifest(paths, sessionId);
...
async function archiveRawAttempt(paths, sessionId, manifestState, onPhase) {
    chmodOwnerReadOnly(paths.raw);
    ...
    try {
        ...
        if (initialIdentity.dev !== pendingIdentity.dev
        ...
        if (finalStat.size !== pendingDigest.stat.size
...
async function recoverArchivePending(paths, sessionId, olderThanDays, now, onPhase) {
    const pending = listPending(paths, sessionId);
    const manifest = loadLatestManifest(paths, sessionId);
    if (pending.length > 1 || (pending.length === 1 && existsSync(paths.raw))) {
        throw new TranscriptStorageError('transcript_ambiguous_state', `ambiguous raw/pending state for ${sessionId}`);
    ...
    try {
        ...
        const initialIdentity = fstatSync(retainedFd, { bigint: true });
        const digest = await digestPlainFile(item.path, true);
        if (initialIdentity.dev !== digest.stat.dev
        ...
        if (existsSync(segmentPath)) {
            try {
                ...
                await onPhase?.('afterPendingVerified');
                const finalStat = fstatSync(retainedFd, { bigint: true });
                if (finalStat.size !== digest.stat.size
                    || finalStat.mtimeNs !== digest.stat.mtimeNs
                    || finalStat.ctimeNs !== digest.stat.ctimeNs) {
                    restorePendingAfterChange(paths, item.path);
                    return null;
                }
                return await commitRecoveredPending(paths, sessionId, item, manifest, segmentName, digest.bytes, onPhase);
            }
            catch (error) {
...
async function commitRecoveredPending(paths, sessionId, item, manifestState, segmentName, sourceBytes, onPhase) {
    const generation = (manifestState?.manifest.generation ?? 0) + 1;
    const segment = {
...
function alreadyArchivedResult(sessionId, manifest) {
...
function beginSourceResolution(sessionId, rootDir) {
    const paths = transcriptPaths(sessionId, rootDir);
    const manifest = loadLatestManifest(paths, sessionId);
...
function finishSourceResolution(sessionId, resolution) {
...
function openSourceSnapshot(sessionId, rootDir) {
    for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
            ...
            if (sourcePlansEqual(before, after))
                return opened;
        }
        catch (error) {
...
async function openSourceSnapshotAsync(sessionId, rootDir, options) {
...
function openSourcePlan(sources) {
    const opened = [];
    try {
        for (const source of sources)
            opened.push({ ...source, fd: openSync(source.path, 'r') });
        return opened;
    }
    catch (error) {
        closeSourcePlan(opened);
...
function closeSourcePlan(sources) {
    for (const source of sources) {
        try {
            closeSync(source.fd);
        }
...
function sourcePlansEqual(left, right) {
...
function isRetryableSnapshotError(error) {
    return isErrno(error, 'ENOENT') || (error instanceof TranscriptStorageError
        && (error.code === 'transcript_ambiguous_state' || error.code === 'transcript_not_found'));
}
function loadLatestManifest(paths, sessionId) {
...
function manifestExtends(previous, current) {
    if (current.segments.length !== previous.segments.length + 1)
        return false;
    return previous.segments.every((segment, index) => segmentEquals(segment, current.segments[index]));
}
function segmentEquals(left, right) {
...
function findManifestFiles(archiveDir) {
...
function parseManifest(path, sessionId, archiveDir) {
    let value;
    try {
    ...
    const segments = value.segments.map((segment, index) => {
        if (!isRecord(segment))
            throw new TranscriptStorageError('transcript_manifest_invalid', `invalid segment schema`);
    ...
    return { version: 1, generation: value.generation, sessionId, segments };
}
function listPending(paths, sessionId) {
...
async function gzipAndValidateFd(fd, displayPath, tempPath) {
    const hash = createHash('sha256');
    let bytes = 0;
    ...
    await pipeline(Readable.from(readFdChunks(fd)), inspect, createGzip({ level: 6 }), createWriteStream(tempPath, { flags: 'wx', mode: 0o600 }));
    return { bytes, sha256: hash.digest('hex'), stat: fstatSync(fd, { bigint: true }) };
}
async function verifyGzipFile(path, expectedSha256, expectedBytes) {
    const hash = createHash('sha256');
    let bytes = 0;
...
async function verifyCommittedPending(paths, sessionId, pending, segment) {
    ...
    await verifyGzipFile(join(paths.archiveDir, segment.fileName), segment.sourceSha256, segment.sourceBytes);
}
async function digestPlainFile(path, validateJson) {
...
function* parseBufferLines(bytes) {
    const text = bytes.toString('utf8');
    const parts = text.split('\n');
    const lastTerminated = text.endsWith('\n');
    for (let index = 0; index < parts.length; index += 1) {
        if (index === parts.length - 1 && parts[index] === '' && lastTerminated)
            continue;
        yield {
...
function verifySourceDigest(source, bytes) {
...
function restorePendingAfterChange(paths, pendingPath) {
    if (existsSync(paths.raw)) {
...
function deleteCommittedPending(path) {
...
function writeNewFileSynced(path, content) {
...
function fsyncDirectory(path) {
    ...
    try {
        ...
        if (!isErrno(error, 'EINVAL') && !isErrno(error, 'EPERM'))
            throw error;
    }
    finally {
        closeSync(fd);
...
function chmodOwnerWritable(path) {
    try {
...
function chmodOwnerReadOnly(path) {
    try {
...
function assertOldEnough(path, olderThanDays, now) {
...
function mapBusyError(error, message) {
