...
export function createYZJWebhookHandler(options) {
    const path = normalizeWebhookPath(options.path);
    return async function handleYZJWebhookRequest(req, res) {
        if (resolvePath(req) !== path)
            return false;
        ...
        jsonOk(res, response);
        void Promise.resolve(options.onMessage(msg)).catch((error) => {
