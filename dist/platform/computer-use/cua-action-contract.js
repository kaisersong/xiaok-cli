...
const CUA_ACTION_CONTRACT_LIST = [
    {
        action: 'capture',
...
export function translateCuaAction(action, publicInput) {
    ...
    for (const [key, value] of Object.entries(publicInput)) {
        if (value === undefined)
            continue;
        if (WRAPPER_ONLY_FIELDS.includes(key) && !(contract.renames && key in contract.renames))
            continue;
...
export function verifyBackendAbi(catalog) {
    ...
    for (const contract of CUA_ACTION_CONTRACTS) {
        ...
        if (JSON.stringify(required) !== JSON.stringify(expected)) {
            problems.push(`${contract.backendOperation} required set is [${required.join(',')}], expected [${expected.join(',')}]`);
        }
        for (const field of contract.translatorAllowed) {
