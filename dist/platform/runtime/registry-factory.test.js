...
import { createPlatformRegistryFactory } from './registry-factory.js';
// Mock dependencies
vi.mock('./context.js', () => ({
    createPlatformRuntimeContext: vi.fn(),
}));
function mockRegistry(tools) {
    const toolMap = new Map();
    for (const t of tools)
        toolMap.set(t.definition.name, t);
    return {
        getToolDefinitions: () => [...toolMap.values()].map(t => t.definition),
    };
}
const mockTools = [
...
describe('registry-factory CC tool filtering', () => {
    ...
    beforeEach(() => {
        ...
        factory = createPlatformRegistryFactory({
            ...
            adapter: () => ({ name: 'test', generate: vi.fn(), stream: vi.fn() }),
