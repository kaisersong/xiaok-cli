...
import { closeSync, constants as fsConstants, fstatSync, fsyncSync, lstatSync, mkdirSync, openSync, readSync, renameSync, rmSync, statSync, writeFileSync, writeSync, } from 'node:fs';
...
export class ProviderOutputStore {
    ...
    promote(request) {
        ...
        if (!preStat)
            throw new InvalidProviderOutputError('candidate does not exist');
        if (preStat.isSymbolicLink())
            throw new InvalidProviderOutputError('candidate is a symlink');
        if (preStat.isDirectory())
            throw new InvalidProviderOutputError('candidate is a directory');
        if (!preStat.isFile())
            throw new InvalidProviderOutputError('candidate is not a regular file');
        ...
        try {
            ...
            if (!intakeLstat.isFile())
                throw new InvalidProviderOutputError('intake is not a regular file');
            if (intakeLstat.nlink !== 1)
                throw new InvalidProviderOutputError('intake is a hardlink');
