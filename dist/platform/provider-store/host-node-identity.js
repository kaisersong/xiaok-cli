...
export const SUPPORTED_HOST_NODE_RANGE = Object.freeze({
    minNodeMajor: 22,
...
export class HostRuntimeIdentityDriftError extends Error {
    field;
    code = 'host_runtime_identity_drift';
    constructor(field) {
 ...
 */
export function deriveAppBundleRoot(execPathRealpath, platform) {
    if (platform !== 'darwin')
        return dirname(execPathRealpath);
...
function hashRegularFile(path, bundleRoot) {
    ...
    if (!st.isFile())
        throw new HostRuntimeBlockedError(`interpreter input is not a regular file: ${resolved}`);
    if (st.nlink !== 1)
        throw new HostRuntimeBlockedError(`interpreter input is a hardlink: ${resolved}`);
    return {
        path: resolved,
...
export function interpreterInputPaths(execPathRealpath, appBundleRoot, platform) {
    const paths = [execPathRealpath];
    if (platform === 'darwin') {
        const versionsA = join(appBundleRoot, 'Contents', 'Frameworks', 'Electron Framework.framework', 'Versions', 'A');
        ...
        if (existsSync(libraries)) {
            for (const name of readdirSync(libraries).sort()) {
                ...
                if (name.endsWith('.dylib'))
                    paths.push(join(libraries, name));
            }
        }
        return paths;
    }
...
export function resolveHostNodeIdentity(input) {
...
export function assertSupportedHostNode(identity, range = SUPPORTED_HOST_NODE_RANGE) {
    ...
    if (!Number.isInteger(major) || major < range.minNodeMajor || major >= range.maxNodeMajorExclusive) {
        throw new HostRuntimeBlockedError(`node ${identity.nodeVersion} is outside the supported range `
 ...
 */
export function checkHostIdentityBeforeSpawn(frozen, now, lastFullHashAt, live) {
    if (live.nodeVersion !== frozen.nodeVersion)
        return { kind: 'drift', field: 'nodeVersion' };
    if (live.moduleAbi !== frozen.moduleAbi)
        return { kind: 'drift', field: 'moduleAbi' };
    if (live.v8Version !== frozen.v8Version)
        return { kind: 'drift', field: 'v8Version' };
    if (live.appVersion !== frozen.appVersion)
        return { kind: 'drift', field: 'appVersion' };
    let realpath;
    try {
    ...
    if (realpath !== frozen.execPathRealpath)
        return { kind: 'drift', field: 'execPathRealpath' };
    ...
    for (const entry of frozen.inputs) {
        ...
        if (!st)
            return { kind: 'drift', field: `missing:${basename(entry.path)}` };
        ...
        if (needsFullHash) {
            ...
            if (sha !== entry.sha256)
                return { kind: 'drift', field: `content:${basename(entry.path)}` };
        }
    }
    return { kind: 'reuse' };
}
