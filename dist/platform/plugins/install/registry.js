...
export const DEFAULT_REGISTRY_V2_URL = 'https://raw.githubusercontent.com/kaisersong/kai-xiaok-plugins/main/registry-v2.json';
...
export function normalizePluginRepository(repo) {
...
function parseInstallStep(raw, index) {
    ...
    try {
        ...
        throw new Error(`Install step #${index} has an invalid "cwd": ${error.message}`);
    }
    if (kind === 'npm_ci') {
        return { kind, cwd };
    }
    ...
    if (kind === 'python_requirements') {
        ...
        try {
            ...
            throw new Error(`Install step #${index} has an invalid requirements "file": ${error.message}`);
        }
    ...
    if (kind === 'external') {
        ...
        const step = { kind, cwd, serverNames: names };
...
function parsePluginEntry(raw, index) {
    ...
    try {
        ...
        throw new Error(`Plugin "${name}" has an invalid "path": ${error.message}`);
    }
...
export function parseTrustedRegistry(raw) {
    const doc = requireObject(raw, 'Registry document');
    if (doc.version !== 2) {
        throw new Error(`Plugin install requires registry v2 (got version ${JSON.stringify(doc.version)}). ` +
...
export async function fetchTrustedRegistryDocument(options = {}) {
    ...
    if (isCustom && !options.trustRegistry) {
        throw new Error('Refusing to install from a custom plugin registry without --trust-registry. ' +
    ...
    for (let redirect = 0; redirect <= maxRedirects; redirect += 1) {
        ...
        if (response.statusCode >= 300 && response.statusCode < 400) {
            ...
            if (next.protocol !== 'https:' || next.host !== expectedHost) {
                throw new Error(`Refusing registry redirect to a different origin: ${next.protocol}//${next.host} (expected https://${expectedHost})`);
            }
        ...
        try {
            ...
            throw new Error(`Plugin registry is not valid JSON: ${error.message}`);
        }
        return parseTrustedRegistry(json);
    }
    throw new Error('Too many registry redirects');
}
const httpsRegistryRequest = (url, maxBytes) => new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'xiaok-cli', Accept: 'application/json' } }, (res) => {
        const chunks = [];
        let size = 0;
        res.on('data', (chunk) => {
