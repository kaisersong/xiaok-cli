...
import { DELIVERY_INPUT_LIMIT, DELIVERY_OUTPUT_LIMIT, DeliveryVerificationError, deliveryFactsSnapshot, encodeDeliveryFrame, parseDeliveryFrame, validateDeliveryFactsV1, validateDeliveryResponse, } from './delivery-facts.js';
// Fixed CPU-only entry. Closing the input port allows a real native exit.
parentPort?.once('message', (frame) => {
    let requestId = '00000000-0000-4000-8000-000000000000';
    let response;
    try {
        const request = parseDeliveryFrame(frame, DELIVERY_INPUT_LIMIT);
        validateDeliveryFactsV1(request);
