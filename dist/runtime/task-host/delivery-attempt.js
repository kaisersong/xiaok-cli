...
export class HostDeliveryAttempt {
    ...
    readerStarted = false;
    record;
    constructor(source, deadline, startedAt, deadlineAt, now, report) {
        this.source = source;
    ...
    async begin() {
        if (!this.report)
            throw new Error('host_delivery_report_unavailable');
        ...
        if (!isDeepStrictEqual(ack, sent))
            throw new Error('host_delivery_ack_mismatch');
        this.acknowledged = true;
        this.assertActive();
    }
    assertActive() {
        if (!this.controller.signal.aborted && performance.now() >= this.deadline)
            this.abort('delivery_timeout');
        this.controller.signal.throwIfAborted();
    }
    abort(code) {
        if (this.record.hostSettlement === 'committed' || this.controller.signal.aborted)
            return;
        if (this.record.verification === 'pending')
            this.fail(code);
    ...
    pass() {
        this.assertActive();
        if (this.record.verification !== 'pending')
            throw new Error('delivery_decision_already_won');
        this.record = { ...this.record, verification: 'passed', decisionAt: this.now() };
    }
    unknown() {
        if (!this.acknowledged || this.record.status === 'unknown' || this.record.hostSettlement === 'committed')
            return;
    ...
    publish(advance = true) {
        if (!this.acknowledged || !this.report)
            return;
        if (advance)
        ...
        let report;
        try {
            report = this.report(this.capture());
        }
        catch {
            return;
        }
