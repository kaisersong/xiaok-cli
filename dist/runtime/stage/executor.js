...
export async function executeStagedSkill(userInput, deps) {
    const debugEvents = [];
    // === Intent Analysis ===
    const t0 = now();
...
async function executeStageWithCheckpoint(stage, skill, deps, stageLabel, debugEvents) {
...
async function executeInSubagent(stage, skill, deps) {
    ...
    try {
        ...
        const prompt = `Execute the skill "${stage.skill}" for this request: ${stage.title}${stage.inputFiles?.length ? `\nInput files: ${stage.inputFiles.join(', ')}` : ''}`;
        const agentDef = {
            name: `stage-${stage.id}`,
            systemPrompt,
...
function buildStageSystemPrompt(stage, skill) {
    const lines = [
...
export function shouldUseSubagent(currentTokens, maxTokens, skillContentSize, skillRefsSize) {
...
function totalRefsSize(refs) {
    return refs.reduce((sum, r) => sum + r.size, 0);
}
function estimateSkillLoadTime(skill) {
    if (!skill)
        return 0;
...
function createCheckpointWarning(stageLabel, stageTitle, modifiedFiles, checkpointId) {
    return {
        timestamp: Date.now(),
        phase: 'checkpoint',
        stage: stageLabel,
 ...
 */
export function analyzeIntent(userInput, skills, cwd) {
    const stages = [];
...
function extractStageTitle(userInput, keyword) {
...
function findBestSkillMatch(input, skills) {
    let bestScore = 0;
    let bestSkill = null;
