...
export class JsonHarnessMemoryStore {
    ...
    promote(id, input) {
        ...
        if (index < 0)
            throw new Error(`harness memory not found: ${id}`);
        const next = {
            ...doc.records[index],
            status: 'active',
    ...
    persist(doc) {
        ...
        try {
            ...
            throw new Error(`harness memory write failed: ${error.message}`);
        }
        finally {
            if (lockFd !== null)
                closeSync(lockFd);
...
function scopeMatches(recordScope, requested) {
    if (recordScope.repo && recordScope.repo !== requested.repo)
        return false;
    if (recordScope.projectId && recordScope.projectId !== requested.projectId)
        return false;
    if (recordScope.runtime && recordScope.runtime !== requested.runtime)
        return false;
    return true;
}
