...
export function markSessionOwned(ledger, instanceId, now = Date.now()) {
...
export function releaseSessionOwnership(ledger, instanceId, now = Date.now()) {
    const next = cloneSessionIntentLedger(ledger);
...
export function resumeSessionOwnership(ledger, instanceId, now = Date.now()) {
    const next = cloneSessionIntentLedger(ledger);
    const currentOwner = next.ownership.ownerInstanceId;
    const previousOwner = next.ownership.previousOwnerInstanceId;
    if (currentOwner && currentOwner !== instanceId && next.ownership.state !== 'released') {
...
export function assertSessionWriteOwnership(ledger, instanceId, action = 'write', options = {}) {
...
export function takeoverSessionOwnership(ledger, instanceId, options = {}) {
