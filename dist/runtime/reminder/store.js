...
export class SQLiteReminderStore {
    ...
    `).run(reminderId, input.sessionId, input.creatorUserId, input.content, input.scheduleAt, input.timezone, input.channel, input.deliveryPolicy ?? 'bound_session', JSON.stringify(input.deliveryTarget), input.taskType ?? 'reminder', input.recurrence ? JSON.stringify(input.recurrence) : null, input.execution ? JSON.stringify(input.execution) : null, idempotencyKey, input.maxRetry ?? 5, input.scheduleAt, now, now);
        return this.getReminder(reminderId);
    }
    getReminder(reminderId) {
        const row = this.db.prepare('SELECT * FROM reminders WHERE reminder_id = ?').get(reminderId);
        return row ? mapReminder(row) : undefined;
    }
    listRemindersForCreator(sessionId, creatorUserId, limit = 20) {
        const rows = this.db.prepare(`
      SELECT * FROM reminders
      WHERE session_id = ? AND creator_user_id = ?
      ORDER BY created_at DESC
      LIMIT ?
    `).all(sessionId, creatorUserId, limit);
        return rows.map(mapReminder);
    }
    listTasksForCreator(sessionId, creatorUserId, limit = 50) {
        const rows = this.db.prepare(`
      SELECT * FROM reminders
    ...
    cancelReminder(reminderId, creatorUserId, now) {
    ...
    cancelTaskChain(reminderId, creatorUserId, now) {
        const source = this.getReminder(reminderId);
        if (!source || source.creatorUserId !== creatorUserId)
            return 0;
        let count = 0;
    ...
    createDeliveryAttempt(reminderId, now) {
    ...
    getDelivery(attemptId) {
        const row = this.db.prepare('SELECT * FROM reminder_deliveries WHERE attempt_id = ?').get(attemptId);
        return row ? mapDelivery(row) : undefined;
    }
    listDeliveries(reminderId) {
        const rows = this.db.prepare(`
    ...
    markDeliverySent(attemptId, now, providerMessageId) {
        this.db.prepare(`
    ...
    markDeliveryFailed(attemptId, now, error) {
        this.db.prepare(`
      UPDATE reminder_deliveries
    ...
    markReminderSent(reminderId, now) {
        this.db.prepare(`
      UPDATE reminders
    ...
    cloneNextOccurrence(reminderId, now) {
        const source = this.getReminder(reminderId);
        if (!source?.recurrence)
            return undefined;
        const rec = source.recurrence;
        const nextOccurrence = rec.occurrenceCount + 1;
        if (rec.maxOccurrences && nextOccurrence >= rec.maxOccurrences)
            return undefined;
        if (rec.endAt && source.scheduleAt + rec.intervalMs > rec.endAt)
            return undefined;
    ...
    `).run(cloneId, source.sessionId, source.creatorUserId, source.content, nextScheduleAt, source.timezone, source.channel, source.deliveryPolicy, JSON.stringify(source.deliveryTarget), source.taskType, JSON.stringify(newRecurrence), source.execution ? JSON.stringify(source.execution) : null, idempotencyKey, source.maxRetry, nextScheduleAt, now, now);
        return this.getReminder(cloneId);
    }
    updateExecution(reminderId, now, status, resultSummary) {
        const reminder = this.getReminder(reminderId);
        if (!reminder?.execution)
            return;
        const updated = {
            ...reminder.execution,
            lastExecutedAt: now,
    ...
    markReminderRetry(reminderId, now, nextAttemptAt, error) {
        this.db.prepare(`
      UPDATE reminders
    ...
    markReminderFailed(reminderId, now, error) {
        this.db.prepare(`
      UPDATE reminders
      SET status = 'failed',
          retry_count = retry_count + 1,
    ...
    recoverStaleDelivering(now, staleAfterMs) {
        const rows = this.db.prepare(`
      SELECT reminder_id FROM reminders
    ...
    migrateSchema() {
        const columns = new Set(this.db.prepare('PRAGMA table_info(reminders)').all().map((row) => row.name));
...
function mapReminder(row) {
    return {
        ...
        deliveryTarget: JSON.parse(row.delivery_target),
        taskType: row.task_type || 'reminder',
        recurrence: row.recurrence ? JSON.parse(row.recurrence) : undefined,
        execution: row.execution ? JSON.parse(row.execution) : undefined,
        status: row.status,
