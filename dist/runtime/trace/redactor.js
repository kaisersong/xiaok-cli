...
export function redactString(input, fieldPath) {
    let value = input;
    const counts = new Map();
    const record = (type, count = 1) => {
...
export function redactTraceValue(input, fieldPath = '') {
    if (typeof input === 'string')
        return redactString(input, fieldPath);
...
function replaceAndCount(input, pattern, replacement, onMatch) {
