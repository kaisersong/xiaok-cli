import { DEFAULT_GOAL_TURN_LIMIT, } from '../runtime/goal/index.js';
...
export function parseGoalSlashCommand(input) {
    ...
    if (rest === 'cancel')
        return { kind: 'cancel' };
    if (rest === 'replace')
        return { kind: 'invalid', message: '用法：/goal replace <objective>' };
...
export function isUnsupportedSingleShotGoalInput(input) {
    return parseGoalSlashCommand(input) !== null;
}
export function formatGoalPreview(input, replacing = false) {
    return [
...
export function formatGoalSummaryLine(goal) {
    if (!goal)
        return '';
    return `Goal · ${goal.status} · ${goal.turnsUsed}/${goal.budgetLimits.turnLimit} turns`;
}
export function buildGoalContinuationInput(goal) {
    return {
